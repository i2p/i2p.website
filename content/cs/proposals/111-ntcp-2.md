...
odolnosti [NTCP]_ vůči různým formám automatické identifikace a útokům.
...
Data [NTCP]_ jsou šifrována po první zprávě (a první zpráva
...
Autoři uznávají, že standardní bezpečnostní praktiky by navrhovaly použití existujícího protokolu jako TLS, ale toto je [Prop104]_ a má vlastní problémy. Kde je to vhodné, byly přidány odstavce „budoucí práce“, aby naznačily chybějící funkce nebo předměty diskuse.


## Cíle Designu

- Podporovat NTCP 1 a 2 na jednom portu, automaticky rozpoznat a publikovat jako jeden
  „transport“ (tj. [RouterAddress]_) v [NetDB]_.
...
- Opravit ztrátu bitů v DH kvůli Java formátu [Ticket1112]_, možná (pravděpodobně?)
  přepnutím na X25519.
...
- Pokračovat v použití podpisů s proměnlivým typem a délkou (z
  zveřejněného [RouterIdentity]_ podepisovacího klíče) jako součást autentizace.
...
- Pokud je to možné, zredukovat handshake skládající se ze 4 zpráv a dvou cest na
  handshake skládající se ze 3 zpráv a jedné cesty, jako v [SSU]_.
...
- Transport na bázi TLS (nebo podobný HTTPS)... to by byl [Prop104]_.
...
je také základem pro protokol [SSU]_.
...
[NOISE]_ (Revize 33, 2017-10-04).
Noise má podobné vlastnosti jako protokol Station-To-Station
[STS]_, který je základem protokolu [SSU]_. V terminologii Noise je Alice
...
- DH Funkce: X25519
  X25519 DH s délkou klíče 32 bytů, jak je specifikováno v [RFC-7748]_.
...
Pro ChaCha20 to, co je zde popsáno, odpovídá [RFC-7539]_, které je také
podobně použito v TLS [RFC-7905]_.
...
pomocí HMAC-SHA256(key, data) podle [RFC-2104]_.
...
Toto je „e“ vzor zprávy:
  ...
  Definuj HMAC-SHA256(key, data) jak je v [RFC-2104]_
