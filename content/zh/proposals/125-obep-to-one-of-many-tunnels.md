...
此提案的委托部分的一个替代方案是通过发送一个[LeaseSet]_哈希，该哈希类似于现有的指定目标[RouterIdentity]_哈希的能力。这将导致消息更小并且可能出现更新的LeaseSet。然而：
...
4. 指定LeaseSet会向OBEP透露消息的[Destination]_，否则他们只能通过在网络中抓取所有LeaseSet并寻找匹配的Lease来发现。

## 设计

发起者（OBGW）将在传递指令[TUNNEL-DELIVERY]_放置一些（全部？）目标[Leases]_，而不是只选择一个。

OBEP将选择其中一个进行传递。OBEP如果可用，将选择一个它已经连接或已知的。这将使OBEP-IBGW路径更快更可靠，并减少整体网络连接。

我们有一个未使用的传递类型（0x03）和两个剩余位（0和1）在[TUNNEL-DELIVERY]_的标志中，我们可以利用这些来实现这些功能。

## 安全影响

此提案不会改变有关OBGW的目标Destination或他们对NetDB的视图所泄露的信息量：

- 控制OBEP并从NetDB抓取LeaseSet的对手已经可以通过搜索[TunnelId]_ / [RouterIdentity]_对来确定消息是否发送到某个特定Destination。最坏情况下，TMDI中多个Leases的存在可能会使其在对手数据库中更快找到匹配项。
...
第一片段传递指令[TUNNEL-DELIVERY]_将修改如下：
...
唯一需要理解新规范的节点是OBGWs和OBEPs。因此，我们可以通过使其使用取决于目标I2P版本[VERSIONS]_，将这次改变与现有网络兼容：

* OBGWs在建立出站隧道时，须基于其[RouterInfo]_中广告的I2P版本选择兼容OBEPs。
