...
本提案描述了一种经过认证的密钥协商协议，以增强 [SSU]_ 对各类自动化识别和攻击的抗性。

提案的组织如下：首先介绍安全目标，然后讨论基本协议。接着给出所有协议消息的完整规范。最后讨论路由器地址和版本识别。

与其他I2P传输一样，SSU2 被定义为I2NP消息的点对点（路由器到路由器）传输。这并不是一个通用数据管道。与 [SSU]_ 一样，它还提供两个额外服务：用于NAT穿透的中继，以及用于确定入站可达性的对等测试。它还提供了SSU中没有的第三个服务，当对等端变更IP或端口时，进行连接迁移。
...
- 在单个端口上支持SSU 1和2，自动检测，并作为单个“传输”（即 [RouterAddress]_) 在 [NetDB]_ 中发布。
...
- 修复因Java格式导致的DH丢失位 [Ticket1112]_，通过切换到X25519来加速DH。
...
- 使用3消息一往返握手，如同 [NTCP2]_. 移除当前消息有效负载等待数据消息导致 [SSU]_ 实际上两往返握手。
...
- 针对DPI的万无一失的抵抗...那会需要插件化的传输，[Prop109]_。

- 基于TLS的（或HTTPS-类似）的传输...那会是 [Prop104]_。
...
Alice和Bob都拥有一个静态密钥对，包含在他们的[RouterIdentity]_中。
...
本提案尝试提供基于 [STS]_协议的五个需求。注意该协议也是[SSU]_协议的基础。
...
* 威胁模型：来自NTCP2 [NTCP2]_，以及对UDP传输模式相关重要威胁的详细分析通过QUIC [RFC-9000]_ [RFC-9001]_。

* 密码选择：参考自 [NTCP2]_。

* 握手：参考自 [NTCP2]_ 和 [NOISE]_ 的Noise XK。NTCP2在此基础上做了显著简化，这得益于UDP提供的封装（固有的消息边界）。

* 握手瞬态密钥模糊化：自 [NTCP2]_ 改编，但使用 [ECIES]_ 中的ChaCha20代替AES。

* 数据包头：改编自WireGuard [WireGuard]_ 和 QUIC [RFC-9000]_ [RFC-9001]_。

* 数据包头混淆：改编自 [NTCP2]_，但使用 [ECIES]_ 中的ChaCha20替换AES。

* 数据包头保护：改编自QUIC [RFC-9001]_ 和 [Nonces]_。

* Headers用作AEAD的关联数据，见 [ECIES]_。

* 数据包编号：改编自WireGuard [WireGuard]_ 和 QUIC [RFC-9000]_ [RFC-9001]_。
...
* 中继和对等测试数据的签名：来自 [Common]_ 的常见结构规范。

* 块格式：来自 [NTCP2]_ 和 [ECIES]_。

* 填充和选项：来自 [NTCP2]_ 和 [ECIES]_。

* Acks, nacks：改编自QUIC [RFC-9000]_。
...
本提案依据Noise协议框架[NOISE]_（第33修订版，2017-10-04），声明需求。
Noise匹配类似Station-To-Station [STS]_，后者是 [SSU]_协议的基础。在Noise例子中，Alice为发起方，Bob为响应方。
...
- DH函数：X25519
  根据[RFC-7748]_，长度为32字节的X25519 DH。

- 加密函数：ChaChaPoly
  AEAD_CHACHA20_POLY1305根据 [RFC-7539]_ 第2.8节。非长度12字节，前4字节置零。
...
本提案定义了在Noise_XK_25519_ChaChaPoly_SHA256上的以下增强。这些普遍遵循[NOISE]_第13节中的指南。
...
STREAM
    ChaCha20/Poly1305 AEAD组合，见[RFC-7539]_。
...
HKDF(salt, ikm, info, n)
    ...
    符合 [RFC-5869]_, 使用HMAC哈希函数SHA-256如同 [RFC-2104]_。SALT_LEN最长为32字节。
...
对于ChaCha20，所描述内容同 [RFC-7539]_规定，并在TLS中相同 [RFC-7905]_。
...
KDF生成握手阶段round phase中cipher key k来源于DH结果校 HMAC-SHA256(key, data)定义在 [RFC-2104]_。
