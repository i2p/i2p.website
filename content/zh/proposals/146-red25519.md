...
[Prop123]_ 定义了一种加密的 LeaseSet2 格式，体现了最小授权原则：每个网络参与者仅获取角色所需的信息。特别是，发布到 floodfill 的加密 LeaseSet2 不会暴露它的目的地，并且只能由事先知道目的地的人查看。但是，floodfill 仍然需要能够在发布加密 LeaseSet2 时进行身份验证，客户端还需要确保身份验证是由目的地本身执行的。
...
本提案中描述的签名方案 Red25519 是 [RedDSA]_ 的一个实例，它是一种支持密钥重新随机化的基于 Schnorr 的签名方案。它具有以下功能：
...
B
    [RFC-8032]_ 中的 Ed25519 基点。

L
    [RFC-8032]_ 中的 Ed25519 阶数 2^252 + 27742317777372353535851937790883648493。
...
为了便于实现，我们在下面明确写出了已经为 Red25519 专门化的 RedDSA 函数（以及若干辅助函数）。实现者应参考 [RedDSA]_ 第 5.4.6 节了解 RedDSA 函数的一般规范。
...
注意，``CONVERT_ED25519_PRIVATE`` 的实现等同于从 [RFC-8032]_ 第 5.1.5 节步骤 1-3 中的 Ed25519 私钥推导 Ed25519 公钥时的秘密标量 s 的计算。
