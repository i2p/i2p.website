...
网络部署和测试正在进行。
可能会有轻微修订。
参见 [specification](/en/docs/spec/red25519/) 获取官方规范。
...
[Proposal 123](/en/proposals/123-new-netdb-entries/) 定义了一种加密的 LeaseSet2 格式，体现了最小授权原则：每个网络参与者仅获取角色所需的信息。特别是，发布到 floodfill 的加密 LeaseSet2 不会暴露它的目的地，并且只能由事先知道目的地的人查看。但是，floodfill 仍然需要能够在发布加密 LeaseSet2 时进行身份验证，客户端还需要确保身份验证是由目的地本身执行的。
