...
我们在2022年开始研究PQ密码学的影响 [FORUM]_。

TLS标准在过去两年中增加了对混合集成加密的支持，现在由于Chrome和Firefox的支持，它已用于互联网上相当一部分加密流量 [CLOUDFLARE]_。

NIST最近最终确定并发布了后量子密码学的推荐算法 [NIST-PQ]_。
几个常见的密码学库现在支持NIST标准或将在不久的将来发布支持。

[CLOUDFLARE]_ 和 [NIST-PQ]_ 都建议立即开始迁移。
另见2022年NSA PQ FAQ [NSA-PQ]_。
I2P应该成为安全和密码学的领导者。
现在是实施推荐算法的时候了。
...
协议 / 功能                       状态
==================================  ======
混合MLKEM棘轮和LS                 已批准2026-06; beta目标2025-08; 发布目标2025-11
混合MLKEM NTCP2                   细节待定
混合MLKEM SSU2                   细节待定
MLDSA SigTypes 12-14             提案稳定但可能不会在2026年之前最终确定
MLDSA Dests                      经过现场测试，要求网络升级以支持洪流填充
混合签名类型 15-17                初步
混合目的
==================================  ======
...
我们将支持基于、但不兼容于CRYSTALS-Kyber和CRYSTALS-Dilithium（版本3.1, 3及更早版本）的NIST FIPS 203和204标准 [FIPS203]_ [FIPS204]_ 。
...
我们将定义三个ML-KEM变体 [FIPS203]_，共计三种新加密类型。
混合类型仅在与X25519结合时定义。
...
我们将使用“防护”或随机签名变体，
而不是“确定性”变体，如 [FIPS204]_ 第3.4节中定义。
...
X.509证书和其他DER编码将使用 [COMPOSITE-SIGS]_ 中定义的组合结构和OID。
...
- ML-KEM（前称CRYSTALS-Kyber）[FIPS203]_
- ML-DSA（前称CRYSTALS-Dilithium）[FIPS204]_
...
================    ================= ======  =====
  类型              公钥长度          起始    用途
================    ================= ======  =====
MLKEM512_X25519               32      0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM768_X25519               32      0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM1024_X25519              32      0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM512                     800      0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM768                    1184      0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM1024                   1568      0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM512_CT                  768      0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM768_CT                 1088      0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM1024_CT                1568      0.9.xx  仅用于握手，不用于租约集、RIs或目的
NONE                           0      0.9.xx  仅用于具有PQ签名类型的目的，不用于RIs或租约集
================    ================= ======  =====

混合公钥是X25519密钥。
KEM公钥是从Alice发送到Bob的临时PQ密钥。
编码和字节顺序在 [FIPS203]_ 中定义。
...
MLKEM768_X25519               32       0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM1024_X25519              32       0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM512                    1632       0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM768                    2400       0.9.xx  仅用于握手，不用于租约集、RIs或目的
MLKEM1024                   3168       0.9.xx  仅用于握手，不用于租约集、RIs或目的
================    ================== ======  =====

混合私钥是X25519密钥。
KEM私钥仅供Alice使用。
KEM编码和字节顺序在 [FIPS203]_ 中定义。
...
============================   ==============  ======  =====
         类型                  长度（字节）     起始    用途
============================   ==============  ======  =====
MLDSA44                              1312      0.9.xx  查看提案169
MLDSA65                              1952      0.9.xx  查看提案169
MLDSA87                              2592      0.9.xx  查看提案169
MLDSA44_EdDSA_SHA512_Ed25519         1344      0.9.xx  查看提案169
MLDSA65_EdDSA_SHA512_Ed25519         1984      0.9.xx  查看提案169
MLDSA87_EdDSA_SHA512_Ed25519         2624      0.9.xx  查看提案169
MLDSA44ph                            1344      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA65ph                            1984      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA87ph                            2624      0.9.xx  仅用于SU3文件，不用于netdb结构
============================   ==============  ======  =====

混合签名公钥是Ed25519密钥后接PQ密钥，如 [COMPOSITE-SIGS]_ 所述。
编码和字节顺序在 [FIPS204]_ 中定义。
...
============================   ==============  ======  =====
         类型                  长度（字节）     起始    用途
============================   ==============  ======  =====
MLDSA44                              2560      0.9.xx  查看提案169
MLDSA65                              4032      0.9.xx  查看提案169
MLDSA87                              4896      0.9.xx  查看提案169
MLDSA44_EdDSA_SHA512_Ed25519         2592      0.9.xx  查看提案169
MLDSA65_EdDSA_SHA512_Ed25519         4064      0.9.xx  查看提案169
MLDSA87_EdDSA_SHA512_Ed25519         4928      0.9.xx  查看提案169
MLDSA44ph                            2592      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA65ph                            4064      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA87ph                            4928      0.9.xx  仅用于SU3文件，不用于netdb结构
============================   ==============  ======  =====

混合签名私钥是Ed25519密钥后接PQ密钥，如 [COMPOSITE-SIGS]_ 所述。
编码和字节顺序在 [FIPS204]_ 中定义。
...
============================   ==============  ======  =====
         类型                  长度（字节）     起始    用途
============================   ==============  ======  =====
MLDSA44                              2420      0.9.xx  查看提案169
MLDSA65                              3309      0.9.xx  查看提案169
MLDSA87                              4627      0.9.xx  查看提案169
MLDSA44_EdDSA_SHA512_Ed25519         2484      0.9.xx  查看提案169
MLDSA65_EdDSA_SHA512_Ed25519         3373      0.9.xx  查看提案169
MLDSA87_EdDSA_SHA512_Ed25519         4691      0.9.xx  查看提案169
MLDSA44ph                            2484      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA65ph                            3373      0.9.xx  仅用于SU3文件，不用于netdb结构
MLDSA87ph                            4691      0.9.xx  仅用于SU3文件，不用于netdb结构
============================   ==============  ======  =====

混合签名是Ed25519签名后接PQ签名，如 [COMPOSITE-SIGS]_ 所述。
...
============================  ===========  =======================  ======  =====
        类型                   类型代码    总公钥长度                起始    用途
============================  ===========  =======================  ======  =====
MLDSA44                           12                 1312           0.9.xx  查看提案169
MLDSA65                           13                 1952           0.9.xx  查看提案169
MLDSA87                           14                 2592           0.9.xx  查看提案169
MLDSA44_EdDSA_SHA512_Ed25519      15                 1344           0.9.xx  查看提案169
MLDSA65_EdDSA_SHA512_Ed25519      16                 1984           0.9.xx  查看提案169
MLDSA87_EdDSA_SHA512_Ed25519      17                 2624           0.9.xx  查看提案169
MLDSA44ph                         18                  n/a           0.9.xx  仅用于SU3文件
MLDSA65ph                         19                  n/a           0.9.xx  仅用于SU3文件
MLDSA87ph                         20                  n/a           0.9.xx  仅用于SU3文件
============================  ===========  =======================  ======  =====
...
================    ===========  ======================= ======  =====
  类型             类型代码      总公钥长度               起始    用途
================    ===========  ======================= ======  =====
MLKEM512_X25519          5                 32            0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM768_X25519          6                 32            0.9.xx  仅用于租约集，不用于RIs或目的
MLKEM1024_X25519         7                 32            0.9.xx  仅用于租约集，不用于RIs或目的
NONE                   255                  0            0.9.xx  查看提案169
================    ===========  ======================= ======  =====
...
我们定义如下函数对应于使用的加密基本构建块，如 [FIPS203]_ 定义。
...
================    =========  =====  =========  =============  =============  ==========  =======
  类型              类型代码   X len  消息1 len  消息1 Enc len  消息1 Dec len  PQ key len  pl len
================    =========  =====  =========  =============  =============  ==========  =======
...
================    =========  =====  =========  =============  =============  ==========  =======
  类型              类型代码   Y len  消息2 len  消息2 Enc len  消息2 Dec len  PQ CT len   opt len
================    =========  =====  =========  =============  =============  ==========  =======
...
注意：虽然消息2通常具有一个非零载荷，但棘轮规范 [ECIES]_ 并不要求它，所以最小载荷大小为0。
最小消息2大小可以相应计算。

### NTCP2

按如下更新NTCP2规范 [NTCP2]_：
...
================    =========  =====  =========  =============  =============  ==========  =======
  类型              类型代码   X len  消息1 len  消息1 Enc len  消息1 Dec len  PQ key len  opt len
================    =========  =====  =========  =============  =============  ==========  =======
...
================    =========  =====  =========  =============  =============  ==========  =======
  类型              类型代码   Y len  消息2 len  消息2 Enc len  消息2 Dec len  PQ CT len   pl len
================    =========  =====  =========  =============  =============  ==========  =======
X25519                   4       32     80+pl        16+pl             pl         --         pl
...
====================  ===================  ============  ======
  类型                相对DH/encaps速度   DH/decaps     keygen
====================  ===================  ============  ======
X25519                     基线           基线          基线
MLKEM512                   29x 更快       22x 更快      17x 更快
MLKEM768                   17x 更快       14x 更快      9x 更快
MLKEM1024                  12x 更快       10x 更快      6x 更快
====================  ===================  ============  ======
...
============================  =======  ====  =======  ======  ======  ========  =====
        类型                   公钥   签名   Key+Sig RIdent 目的   RInfo     LS/流/数据报（每个消息）
============================  =======  ====  =======  ======  ======  ========  =====
