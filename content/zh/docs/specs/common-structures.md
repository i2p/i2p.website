...
- 可压缩填充规范已最终确定（[提案 161](/proposals/161-ri-dest-padding/)，在 0.9.57 中实现）
...
```
### 填充生成指南 ([Proposal 161](/proposals/161-ri-dest-padding/))
...
3. **典型大小：**
   ...
   - 32 字节 X25519 公钥
   - 320 字节填充（可按 [Proposal 161](/proposals/161-ri-dest-padding/) 压缩）
   - 32 字节 EdDSA 公钥
...
**另见：** - 填充生成指南 ([提案 161](/proposals/161-ri-dest-padding/)) - 上文的密钥证书规范
...
2. **加密密钥：**
   - 该字段未被使用，但必须存在
   - **建议：** 根据[提案 161](/proposals/161-ri-dest-padding/)填充随机数据（可压缩）
...
**向后兼容性：** - ElGamal 和 DSA_SHA1 仍支持旧版 routers - 已弃用的密钥类型仍可用，但不建议使用 - 可压缩填充（[Proposal 161](/proposals/161-ri-dest-padding/)）与 0.6 版本向后兼容
...
- [提案 161：填充生成](/proposals/161-ri-dest-padding/)
