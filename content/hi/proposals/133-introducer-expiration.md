```markdown
---
title: "परिचायक समाप्ति"
number: "133"
author: "zzz"
created: "2017-02-05"
lastupdated: "2017-08-09"
status: "बंद"
thread: "http://zzz.i2p/topics/2230"
target: "0.9.30"
implementedin: "0.9.30"
---

## अवलोकन

यह प्रस्ताव परिचयों की सफलता दर में सुधार लाने के बारे में है। देखें
[TRAC-TICKET]_।

## प्रेरणा

परिचायक एक निश्चित समय के बाद समाप्त हो जाते हैं, लेकिन उस जानकारी को
[RouterInfo]_ में प्रकाशित नहीं किया गया है। राउटर को वर्तमान में अनुमान लगाने के लिए
सिद्धांतों का उपयोग करना होता है जब एक परिचायक अब मान्य नहीं होता।

## डिज़ाइन

यदि SSU [RouterAddress]_ में परिचायक शामिल हैं, तो प्रकाशक प्रत्येक परिचायक के
लिए समाप्ति समय वैकल्पिक रूप से शामिल कर सकता है।

## विशिष्टता

.. raw:: html

  {% highlight lang='dataspec' %}
iexp{X}={nnnnnnnnnn}

  X :: परिचायक संख्या (0-2)

  nnnnnnnnnn :: युग से सेकंड्स (मिलीसेकंड्स नहीं) का समय।
{% endhighlight %}

नोट्स
`````
* प्रत्येक समाप्ति को [RouterInfo]_ के प्रकाशन तिथि से अधिक और
  RouterInfo की प्रकाशन तिथि के 6 घंटे बाद से कम होना चाहिए।

* प्रकाशित राउटर और परिचायक की कोशिश करना चाहिए कि परिचायक को समाप्ति
  तक मान्य रखा जाए, हालांकि वे इसकी गारंटी नहीं दे सकते।

* समाप्ति के बाद राउटर प्रकाशित परिचायक का उपयोग नहीं करना चाहिए।

* परिचायक समाप्तियां [RouterAddress]_ मैपिंग में हैं।
  वे [RouterAddress]_ में (वर्तमान में अप्रयुक्त) 8-बाइट समाप्ति क्षेत्र नहीं हैं।

उदाहरण: ``iexp0=1486309470``

## माइग्रेशन

कोई समस्या नहीं। कार्यान्वयन वैकल्पिक है। 
पिछड़ा संगतता सुनिश्चित है, क्योंकि पुराने राउटर अज्ञात पैरामीटरों को अनदेखा करेंगे।


## संदर्भ

.. [RouterAddress]
    {{ ctags_url('RouterAddress') }}

.. [RouterInfo]
    {{ ctags_url('RouterInfo') }}

.. [TRAC-TICKET]
    http://{{ i2pconv('trac.i2p2.i2p') }}/ticket/1352
```
