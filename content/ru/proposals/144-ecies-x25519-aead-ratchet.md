...
Смотрите [SPEC](/en/docs/spec/ecies/) для официальной спецификации.
...
с начала I2P, чтобы заменить ElGamal/AES+SessionTags [Elg-AES](/en/docs/spec/elgamal-aes/).

Оно основывается на предыдущих работах следующим образом:

- Спецификация общих структур [Common](/en/docs/spec/common-structures/)
- Спецификация [I2NP](/en/docs/spec/i2np/) включая LS2
- ElGamal/AES+Session Tags [Elg-AES](/en/docs/spec/elgamal-aes/)
- Обзор нового асимметричного шифрования http://zzz.i2p/topics/1768
- Краткий обзор криптографии [CRYPTO-ELG](/en/docs/how/cryptography/)
...
- Алгоритм двойного ратчета [Signal](https://signal.org/docs/specifications/doubleratchet/)
...
Смотрите спецификацию общих структур [Common](/en/docs/spec/common-structures/).
...
1) Сообщения о построении туннеля (ключ находится в RouterIdentity)
   ...
   Смотрите предложение 152 [Prop152](/en/proposals/152-ecies-tunnels/).
...
- Новое шифрование для шифрования туннельного слоя.
  Смотрите предложение 153 [Prop153](/en/proposals/153-ecies-garlic/).
...
Смотрите таблицу "Payload Security Properties" в [NOISE](https://noiseprotocol.org/noise.html).
...
Существующие реализации маршрутизатора I2P, которые еще не реализовали [NTCP2](/en/docs/transport/ntcp2/) ([Prop111](/en/proposals/111-ntcp2/))
...
Типы шифрования 1-3 зарезервированы для ECIES-ECDH-AES-SessionTag, смотрите предложение 145 [Prop145](/en/proposals/145-ecies/).


### Noise Protocol Framework

Это предложение предоставляет требования на основе Noise Protocol Framework
[NOISE](https://noiseprotocol.org/noise.html) (Ревизия 34, 2018-07-11).
Noise имеет аналогичные свойства протоколу Station-To-Station
[STS](https://en.wikipedia.org/wiki/Station-to-Station_protocol), который является основой для протокола [SSU](/en/docs/transport/ssu/). В терминологии Noise, Alice
...
- Функция шифрования: ChaChaPoly
  ...
  Идентично тому, что в [NTCP2](/en/docs/transport/ntcp2/).
...
[NOISE](https://noiseprotocol.org/noise.html) разделе 13.

1) Уникальные эфемерные ключи кодируются с помощью [Elligator2](https://elligator.org/).
...
Все сообщения включают заголовок Garlic Message [I2NP](/en/docs/spec/i2np/).
...
STREAM
    AEAD ChaCha20/Poly1305, как указано в [RFC-7539](https://tools.ietf.org/html/rfc7539).
...
HKDF(salt, ikm, info, n)
    ...
    Использовать HKDF, как указано в [RFC-5869](https://tools.ietf.org/html/rfc5869), используя хеш-функцию HMAC SHA-256
    как указано в [RFC-2104](https://tools.ietf.org/html/rfc2104). Это означает, что SALT_LEN максимально 32 байта.
...
Garlic Message, как указано в [I2NP](/en/docs/spec/i2np/), следующий.
...
Смотрите [I2NP](/en/docs/spec/i2np/) для подробностей и полной спецификации.
...
Сообщение New Session похоже на односторонний Noise [NOISE](https://noiseprotocol.org/noise.html)  шаблон
...
Это стандартное [NOISE](https://noiseprotocol.org/noise.html) для IK с модифицированной строкой протокола.
...
Обоснование: Используется в [NTCP2](/en/docs/transport/ntcp2/).
...
В [NTCP2](/en/docs/transport/ntcp2/) ([Prop111](/en/proposals/111-ntcp2/)) была применена низкая стоимость xor-функция с использованием ключа из вне для
...
Поэтому используется [Elligator2](https://elligator.org/) для преобразования эфемерных ключей в сообщениях новой сессии и ответах на новую сессию
...
Как определено в [Elligator2](https://elligator.org/), закодированные ключи не отличимы от 254 случайных битов.
...
используются для [NTCP2](/en/docs/transport/ntcp2/), где Elligator2 не используется.
...
AEAD с использованием ChaCha20 и Poly1305, так же, как в [NTCP2](/en/docs/transport/ntcp2/).
Это соответствует [RFC-7539](https://tools.ietf.org/html/rfc7539), который также
...
Используется в [NTCP2](/en/docs/transport/ntcp2/).
