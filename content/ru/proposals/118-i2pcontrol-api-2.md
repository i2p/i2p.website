...
"id"
  Идентификационный номер или идентификатор запроса.

  Используется для идентификации, какой ответ соответствует какому запросу.
...
"auth_token"
  ...
  Должен быть предоставлен для каждого RPC, за исключением вызова 'authenticate'.

"method_parameter_value"
  Параметр метода.

  Используется для предложений разных вариантов метода. Как, например, 'get', 'set' и тому подобные варианты.
