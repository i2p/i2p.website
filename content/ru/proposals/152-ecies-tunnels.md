...
См. [SPEC](/docs/specs/implementation/) для официальной спецификации.


## Обзор

Этот документ предлагает изменения в шифровании сообщений построения туннеля с использованием криптопримитивов, введенных в [ECIES-X25519](/docs/specs/ecies/).
Это часть общей предложения
[Prop156](/proposals/156-ecies-routers/) по переводу маршрутизаторов с ElGamal на ECIES-X25519 ключи.
...
Для обзора всех изменений, необходимых для маршрутизаторов ECIES, см. предложение 156 [Prop156](/proposals/156-ecies-routers/).

Это предложение сохраняет тот же размер записей построения туннелей, как требуется для совместимости. Меньшие записи и сообщения будут реализованы позже - см. [Prop157](/proposals/157-new-tbm/).
...
- Без изменений в соответствии туннелей AES "слоя", для этого см. [Prop153](/proposals/153-chacha20-layer-encryption/)
...
- Использование опций построения туннелей, как определено в [Prop143](/proposals/143-build-message-options/), только необходимо для небольших сообщений
- Двусторонние туннели - для этого см. [Prop119](/proposals/119-bidirectional-tunnels/)
- Меньшие сообщения о построении туннелей - для этого см. [Prop157](/proposals/157-new-tbm/)
...
Это также идентично первому (Сообщение запроса сеанса) в шаблону XK, используемому в [NTCP2](/docs/specs/ntcp2/).
...
Это асимметричное шифрование записей запросов в настоящее время использует ElGamal как определено в [Cryptography](/docs/specs/cryptography/)
...
Для справки, это текущая спецификация туннельного BuildRequestRecord для маршрутизаторов ElGamal, взятая из [I2NP](/docs/specs/i2np/).
...
Опции построения туннеля является структурой сопоставления, как определено в [Common](/docs/specs/common-structures/).
...
Опции ответа построения туннеля является структурой Сопоставления, как определено в [Common](/docs/specs/common-structures/).
Это для будущего использования. В настоящее время никаких опций не определено.
Если структура Сопоставления пуста, это два байта 0x00 0x00.
...
как определено в [Tunnel-Creation](/docs/specs/tunnel-creation/) для избежания отпечатков пальцев:
...
См. [Prop156](/proposals/156-ecies-routers/) для деталей статических ECIES ключей маршрутизаторов.
...
схемы, определенной в [Tunnel-Creation](/docs/specs/tunnel-creation/). Создатели туннелей ECIES будут использовать вышеуказанную схему для шифрования
к ECIES хопам.
...
Как определено в [Tunnel-Creation](/docs/specs/tunnel-creation/).
...
См. [Prop156](/proposals/156-ecies-routers/).
