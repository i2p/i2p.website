...
См. [SPEC]_ для официальной спецификации.


## Обзор

Этот документ предлагает изменения в шифровании сообщений построения туннеля с использованием криптопримитивов, введенных в [ECIES-X25519]_.
Это часть общей предложения
[Prop156]_ по переводу маршрутизаторов с ElGamal на ECIES-X25519 ключи.
...
Для обзора всех изменений, необходимых для маршрутизаторов ECIES, см. предложение 156 [Prop156]_.

Это предложение сохраняет тот же размер записей построения туннелей, как требуется для совместимости. Меньшие записи и сообщения будут реализованы позже - см. [Prop157]_.
...
- AES-256-CBC как в [Cryptography]_
- Функции STREAM ChaCha20/Poly1305:
  ENCRYPT(k, n, plaintext, ad) и DECRYPT(k, n, ciphertext, ad) - как в [NTCP2]_ [ECIES-X25519]_ и [RFC-7539]_
- Функции X25519 DH - как в [NTCP2]_ и [ECIES-X25519]_
- HKDF(salt, ikm, info, n) - как в [NTCP2]_ и [ECIES-X25519]_

Другие функции Noise, определенные в других местах:

- MixHash(d) - как в [NTCP2]_ и [ECIES-X25519]_
- MixKey(d) - как в [NTCP2]_ и [ECIES-X25519]_
...
- Без изменений в соответствии туннелей AES "слоя", для этого см. [Prop153]_
...
- Использование опций построения туннелей, как определено в [Prop143]_, только необходимо для небольших сообщений
- Двусторонние туннели - для этого см. [Prop119]_
- Меньшие сообщения о построении туннелей - для этого см. [Prop157]_
...
- DH функция: X25519
  X25519 DH с длиной ключа 32 байта, как определено в [RFC-7748]_.

- Функция шифрования: ChaChaPoly
  AEAD_CHACHA20_POLY1305, как определено в [RFC-7539]_ раздел 2.8.
...
Рукопожатия используют шаблоны рукопожатия [Noise]_.
...
Это также идентично первому (Сообщение запроса сеанса) в шаблону XK, используемому в [NTCP2]_.
...
Это асимметричное шифрование записей запросов в настоящее время использует ElGamal как определено в [Cryptography]_
...
Для справки, это текущая спецификация туннельного BuildRequestRecord для маршрутизаторов ElGamal, взятая из [I2NP]_.
Данные без шифрования добавляются с ненулевым байтом и SHA-256 хешем данных до шифрования,
как определено в [Cryptography]_.
...
Опции построения туннеля является структурой сопоставления, как определено в [Common]_.
...
Опции ответа построения туннеля является структурой Сопоставления, как определено в [Common]_.
Это для будущего использования. В настоящее время никаких опций не определено.
Если структура Сопоставления пуста, это два байта 0x00 0x00.
...
как определено в [Tunnel-Creation]_ для избежания отпечатков пальцев:
...
На пути ответа конечная точка (отправитель) должна будет отменить [Multiple-Encryption]_, используя каждый ответный ключ маршрутизатора.
...
См. [Prop156]_ для деталей статических ECIES ключей маршрутизаторов.
...
Это стандарт [NOISE]_ для шаблона "N" со стандартным именем протокола.
...
схемы, определенной в [Tunnel-Creation]_. Создатели туннелей ECIES будут использовать вышеуказанную схему для шифрования
к ECIES хопам.
...
Как определено в [Tunnel-Creation]_.
