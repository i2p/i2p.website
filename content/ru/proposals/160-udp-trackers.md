...
Спецификация находится в [UDP]_.
...
Предварительное предложение для UDP трекеров в I2P было размещено на нашей странице спецификации bittorrent [SPEC]_
...
Так как это предложение зависит от реплицируемых дейтаграмм, оно было отложено, как только мы начали работать над предложением Datagram2 [Prop163]_ в начале 2023 года.
...
Этот режим будет легче реализовать для любого трекера или клиента, поддерживающего стандарт [BEP15]_.
...
Поэтому текущая версия здесь еще больше упрощается, чтобы убрать быстрый режим и термин "совместимость". Текущая версия переключается на новый формат Datagram2 и добавляет ссылки на протокол расширения UDP announce [BEP41]_.
...
Bittorrent предложил UDP трекеры в BEP 15 [BEP15]_ в 2008 году, и подавляющее большинство трекеров в clearnet теперь работают только через UDP.
...
Шифрование и подписи после квантового перехода, как это предусмотрено в [Prop169]_, существенно увеличат накладные расходы на зашифрованные и подписанные структуры, включая адреса назначения, наборы аренды, синхронизацию потоков и ACK. Важно минимизировать эти накладные расходы, где это возможно, до принятия PQ криптографии в I2P.

## Дизайн

Это предложение использует реплицируемые дейтаграммы2, реплицируемые дейтаграммы3 и необработанные дейтаграммы, как это определено в [DATAGRAMS]_.
Datagram2 и Datagram3 — новые варианты реплицируемых дейтаграмм, определенные в предложении 163 [Prop163]_.
...
потребуется SAM v3.3 [SAMv3]_, который не поддерживается i2pd.
...
[BEP15]_ указывает, что ID соединения истекает через одну минуту на клиенте и через две минуты на трекере.
...
Этот дизайн сохраняет совместимость с [BEP15]_ по максимуму
...
Формат URL анонса не указан в [BEP15]_,
...
они могут быть обработаны и предоставлены в запросе анонса, как указано в [BEP41]_.
Если нет параметров или пути, завершающий / также может быть опущен,
как подразумевается в [BEP41]_.
...
Клиент для трекера.
16 байт. Должен быть реплицируемым Datagram2. То же, что в [BEP15]_. Без изменений.
...
Трекер для клиента.
16 или 18 байт. Должен быть необработанным. То же, что в [BEP15]_, за исключением отмеченного ниже.
...
Клиент для трекера.
Минимум 98 байт. Должен быть реплицируемым Datagram3. То же, что в [BEP15]_, за исключением отмеченного ниже.
...
Изменения из [BEP15]_:

- ключ игнорируется
- порт вероятно игнорируется
- Секция опций, если присутствует, как определено в [BEP41]_
...
Трекер для клиента.
Минимум 20 байт. Должен быть необработанным. То же, что в [BEP15]_, за исключением отмеченного ниже.
...
Запрос/ответ скрапа из [BEP15]_ не требуется данным предложением,
...
Должен быть необработанным. То же, что в [BEP15]_. Без изменений.
...
Формат расширений, определенный в [BEP41]_, рекомендуется, если требуется.
...
используя формат [BEP41]_ (параметры TBD).
