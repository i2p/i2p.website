...
подобно записям DNS SRV [SRV]_ [RFC2782]_.
...
Предложение LS2 123 [Prop123]_ определило 'записи служб', которые указывали, что адресат
...
GNS [GNS]_ предлагает, чтобы каждый запускал свой собственный DNS сервер.
...
В [DOTWELLKNOWN]_ предлагается, чтобы службы искались через HTTP-запрос на
...
Записи служб помещаются в секцию опций в LS2 [LS2]_.
Секция опций LS2 в настоящее время не используется.
Не поддерживается для LS1.
Это похоже на предложение по пропускной способности туннеля [Prop168]_,
...
- service := Символическое имя желаемой службы. Должно быть в нижнем регистре. Пример: "smtp".
  ...
  Должны быть использованы стандартные идентификаторы из [REGISTRY]_ или Linux /etc/services, если они там определены.
...
- target := Имя хоста или b32 адрес, предоставляющий услугу. Действительное имя хоста, как в [NAMING]_. Должно быть в нижнем регистре.
...
Нестандартные идентификаторы, не перечисленные в [REGISTRY]_ или Linux /etc/services,
могут быть запрошены и добавлены в спецификацию общих структур [LS2]_.
...
Протокол [I2CP]_ должен быть расширен, чтобы поддерживать поиски служб.
...
Протокол [SAMv3]_ должен быть расширен для поддержки поисков служб.
...
обновив [NAMING]_ для указания обработки имен хостов, начинающихся с '_'.
...
За пределами сферы действия этого предложения. См. [DOTWELLKNOWN]_ для обсуждения.
...
Реализации могут добавить поддержку в любое время, координация не нужна,
кроме соглашения об эффективной версии API для изменений I2CP.
Совместимые версии SAM для каждой реализации будут задокументированы в спецификации SAM.
