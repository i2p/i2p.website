...
Мы начали изучать последствия PQ криптографии в 2022 [FORUM]_.

Стандарты TLS добавили поддержку гибридного шифрования за последние два года, и оно теперь используется значительной частью шифрованного трафика в интернете благодаря поддержке в Chrome и Firefox [CLOUDFLARE]_.

NIST недавно завершил и опубликовал рекомендуемые алгоритмы для пост-квантовой криптографии [NIST-PQ]_. Несколько популярных криптографических библиотек теперь поддерживают стандарты NIST или выпустят поддержку в ближайшем будущем.

Оба [CLOUDFLARE]_ и [NIST-PQ]_ рекомендуют начинать миграцию немедленно. См. также FAQ NSA PQ 2022 года [NSA-PQ]_. I2P должен быть лидером в области безопасности и криптографии. Пора внедрять рекомендуемые алгоритмы. Используя нашу гибкую систему типов криптографии и типов подписи, мы добавим типы для гибридной криптографии и для PQ и гибридных подписей.
...
==================================  ======
Гибридный MLKEM Ratchet и LS        Одобрено 2026-06; бета-цель 2025-08; выпускная цель 2025-11
Гибридный MLKEM NTCP2               Некоторые детали требуют завершения
Гибридный MLKEM SSU2                Некоторые детали требуют завершения
MLDSA SigTypes 12–14                Предложение стабильно, но может быть не финализировано до 2026
MLDSA Dests                         Протестировано в живой сети, требует обновления сети для поддержки floodfill
Гибридные SigTypes 15–17            Предварительно
Гибридные Dests
==================================  ======
...
Мы поддержим стандарты NIST FIPS 203 и 204 [FIPS203]_ [FIPS204]_, которые основаны на, но НЕ совместимы с,
...
=======  ==========  ================  ===============
Прото    Тип Noise   Поддержка только PQ?  Поддержка гибрида?
=======  ==========  ================  ===============
NTCP2       XK       нет                да
SSU2        XK       нет                да
Ratchet     IK       нет                да
...
Таким образом, мы будем поддерживать только гибридное шифрование для NTCP2, SSU2 и Ratchet. Мы определим три варианта ML-KEM, как указано в [FIPS203]_, всего для 3 новых типов шифрования. Гибридные типы будут определены только в сочетании с X25519.
...
X.509 сертификаты и другие кодировки DER будут использовать композитные структуры и OID, определенные в [COMPOSITE-SIGS]_.
...
- ML-KEM (ранее CRYSTALS-Kyber) [FIPS203]_
- ML-DSA (ранее CRYSTALS-Dilithium) [FIPS204]_
...
Обновите разделы и таблицы в документе общих структур [COMMON]_ следующим образом:
...
================    ================= ======  =====
  Тип              Длина общественного ключа С     Использование
================    ================= ======  =====
MLKEM512_X25519               32      0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM768_X25519               32      0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM1024_X25519              32      0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM512                     800      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM768                    1184      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM1024                   1568      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM512_CT                  768      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM768_CT                 1088      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM1024_CT                1568      0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
NONE                           0      0.9.xx  См. предложение 169, только для destinations с PQ типами подписей, не для RIs или Leasesets
================    ================= ======  =====

Гибридные общественные ключи – это ключ X25519.
KEM общественные ключи – это эфемерный PQ-ключ, отправленный от Алисы к Бобу.
Кодирование и порядок байт определены в [FIPS203]_.
...
================    ================== ======  =====
  Тип              Длина личного ключа С     Использование
================    ================== ======  =====
MLKEM512_X25519               32       0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM768_X25519               32       0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM1024_X25519              32       0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM512                    1632       0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM768                    2400       0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
MLKEM1024                   3168       0.9.xx  См. предложение 169, только для рукопожатий, не для Leasesets, RIs или Destinations
================    ================== ======  =====

Гибридные частные ключи – это ключи X25519.
KEM частные ключи – только для Алисы.
Кодирование и порядок байт для KEM определены в [FIPS203]_.
...
============================   ==============  ======  =====
         Тип                  Длина (байты)  С     Использование
============================   ==============  ======  =====
...
Гибридные открытые ключи подписания – это ключ Ed25519, за которым следует PQ-ключ, как в [COMPOSITE-SIGS]_. Кодирование и порядок байт определены в [FIPS204]_.
...
MLDSA44ph                            2592      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
MLDSA65ph                            4064      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
MLDSA87ph                            4928      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
============================   ==============  ======  =====

Гибридные частные ключи подписания – это ключ Ed25519, за которым следует PQ-ключ, как в [COMPOSITE-SIGS]_. Кодирование и порядок байт определены в [FIPS204]_.
...
MLDSA44ph                            2484      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
MLDSA65ph                            3373      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
MLDSA87ph                            4691      0.9.xx  Только для файлов SU3, не для структур netdbСм. предложение 169
============================   ==============  ======  =====

Гибридные подписи – это подпись Ed25519, за которой следует подпись PQ, как в [COMPOSITE-SIGS]_.
Гибридные подписи проверяются путем проверки обеих подписей и отказа
если хотя бы одна не проходит.
Кодирование и порядок байт определены в [FIPS204]_.
...
============================  ===========  =======================  ======  =====
        Тип                  Код Типа    Общая длина открытого ключа  С     Использование
============================  ===========  =======================  ======  =====
...
================    ===========  ======================= ======  =====
  Тип              Код Типа    Общая длина открытого ключа С     Использование
================    ===========  ======================= ======  =====
MLKEM512_X25519          5                 32            0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM768_X25519          6                 32            0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
MLKEM1024_X25519         7                 32            0.9.xx  См. предложение 169, только для Leasesets, не для RIs или Destinations
...
============================  ===========  =======================  ======  ======  =====
        Тип                  Код Типа    Общая длина открытого ключа  Основная    Избыточная  Общая длина назначения
============================  ===========  =======================  ======  ======  =====
...
============================  ===========  =======================  ======  ======  =====
        Тип                  Код Типа    Общая длина открытого ключа  Основная    Избыточная  Общая длина RouterIdent
============================  ===========  =======================  ======  ======  =====
...
Обновите спецификацию ECIES-Ratchet [ECIES]_ следующим образом:
...
================    =========  =====  =========  =============  =============  ==========  =======
  Тип              Код Типа  X дл.  Сообщение 1 дл.  Сообщение 1 Закрытая дл.  Сообщение 1 Открытая дл.  Дл. PQ ключ  Дл. полезной нагрузки
================    =========  =====  =========  =============  =============  ==========  =======
...
================    =========  =====  =========  =============  =============  ==========  =======
  Тип              Код Типа  Y длина  Длина сообщения 2  Длина кодового сообщения 2  Длина кодируемого сообщения 2  PQ CT длина  opt длина
================    =========  =====  =========  =============  =============  ==========  =======
...
спецификация ратчета [ECIES]_ этого не требует, поэтому минимальный размер полезной нагрузки равен 0.
...
Обновите спецификацию NTCP2 [NTCP2]_ следующим образом:
...
================    =========  =====  =========  =============  =============  ==========  =======
  Тип              Код Типа  X длина  Длина сообщения 1  Закрытая длина сообщения 1  Открытая длина сообщения 1  Длина PQ ключа  Длина параметра
================    =========  =====  =========  =============  =============  ==========  =======
...
================    =========  =====  =========  =============  =============  ==========  =======
  Тип              Код Типа  Y длина  Длина сообщения 2  Закрытая длина сообщения 2  Открытая длина сообщения 2  Длина PQ CT  Длина параметра
================    =========  =====  =========  =============  =============  ==========  =======
X25519                   4       32     64+pad       32              16           --         16
...
Обновите спецификацию SSU2 [SSU2]_ следующим образом:
...
================    =========  =====  =========  =============  =============  ==========  =======
  Тип              Код Типа  X длина  Длина сообщения 1  Закрытая длина сообщения 1  Открытая длина сообщения 1  Длина PQ ключа  Длина полезной нагрузки
================    =========  =====  =========  =============  =============  ==========  =======
X25519                   4       32     80+pl        16+pl             pl         --         pl
MLKEM512_X25519          5       32    896+pl       832+pl         800+pl        800         pl
MLKEM768_X25519          6       32   1280+pl      1216+pl        1184+pl       1184         pl
MLKEM1024_X25519         7      n/a   слишком большой
================    =========  =====  =========  =============  =============  ==========  =======
