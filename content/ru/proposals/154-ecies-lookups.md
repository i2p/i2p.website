...
См. [I2NP]_ для официальной спецификации.
...
- ElG: ElGamal
- ENCRYPT(k, n, payload, ad): как определено в [ECIES]_
- LS: Leaseset
- lookup: I2NP DLM
...
С пунктами назначения [ECIES]_, цель предложения 144 состоит в том, чтобы
...
- Новый формат DLM добавит бит в поле флагов для указания ответов зашифрованных ECIES.
  Ответы зашифрованные ECIES будут использовать формат Existing Session сообщения [ECIES]_,
...
В спецификации [I2NP]_ DLM (DatabaseLookup) внести следующие изменения.
...
Ответ - это сообщение ECIES Existing Session, как определено в [ECIES]_.
...
Маршрутизаторы ECIES были введены в версии 0.9.48, см. [Prop156](/en/proposals/156-ecies-routers/).
...
Поиск будет использовать "одноразовый формат" в [ECIES]_
...
Ответ - это сообщение ECIES Existing Session, как определено в [ECIES]_.
См. [ECIES]_ для всех определений.
...
// Эфемерные ключи X25519 Алисы
  ...
  tag :: 8 байт тег, сгенерированный от RATCHET_TAG() в [ECIES]_

  k :: 32 байт ключ, сгенерированный от RATCHET_KEY() в [ECIES]_
...
такое же как в [ECIES]_, скопировано ниже для справки.
