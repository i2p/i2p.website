...
См. [I2NP]_ и [Tunnel-Creation-ECIES]_ для окончательной спецификации.
...
Изменения в формате записи размером 528 байт для маршрутизаторов ECIES-X25519 специфицированы в [Prop152]_ и [Tunnel-Creation-ECIES]_.
...
См. [Prop152]_ и [Prop156]_ для дополнительных целей.

- Меньшие записи и сообщения
- Поддержание достаточного места для будущих опций, как в [Prop152]_ и [Tunnel-Creation-ECIES]_
- Вписывание в одно туннельное сообщение для обратного пути
- Поддержка только узлов ECIES
- Поддержание улучшений, реализованных в [Prop152]_ и [Tunnel-Creation-ECIES]_
...
См. [Prop156]_ для дополнительных не-целей.

- Отсутствие требования для смешанных туннелей ElGamal/ECIES
- Изменения шифрования слоя, для этого см. [Prop153]_
...
и записями ECIES, определенными в [Prop152]_ и [Tunnel-Creation-ECIES]_, которые занимают 464 байта.

Записи нешифрованного ответа будут занимать 202 байта,
по сравнению с записями ElGamal, которые занимают 496 байт,
и записями ECIES, определенными в [Prop152]_ и [Tunnel-Creation-ECIES]_, которые занимают 512 байт.
...
Шифрование записей запроса и ответа: как определено в [Prop152]_ и [Tunnel-Creation-ECIES]_.
...
Обзор изменений из [Tunnel-Creation-ECIES]_:
...
Поле флагов такое же как определено в [Tunnel-Creation]_ и содержит следующее:
...
Параметры построения туннеля - это структура Mapping, как определено в [Common]_.
...
Параметры ответа на построение туннеля - это структура Mapping, как определено в [Common]_.
Это для будущего использования. На данный момент не определено никаких параметров.
Если структура Mapping пуста, это два байта 0x00 0x00.
...
как определено в [Tunnel-Creation]_ для предотвращения фингерпринтинга:
