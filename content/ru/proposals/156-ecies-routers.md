...
- ECIES роутеры реализованы в 0.9.48, см. [Common]_.
- Создание туннелей реализовано в 0.9.48, см. [Tunnel-Creation-ECIES]_.
- Зашифрованные сообщения для ECIES роутеров реализованы в 0.9.49, см. [ECIES-ROUTERS]_.
...
Предложения для LS2 [Prop123]_ и ECIES-X25519-AEAD-Ratchet [Prop144]_
(теперь определены в [ECIES]_) описали замену ElGamal на ECIES
для Destinations.
...
См. [Prop152]_ для дополнительных целей.
...
См. [Prop152]_ для дополнительных не-целей.

- Нет требований для роутеров с двойными ключами
- Изменения в уровне шифрования, для этого см. [Prop153]_
...
находится в его Идентификаторе роутера. См. спецификацию общих структур [Common]_.
...
Это стандартное построение, как определено в спецификации общих структур [Common]_.

Это идентично методу, предложенному для ECIES-P256
для типов криптографии 1-3 в предложении 145 [Prop145]_.
...
Для использования ECIES вместо ElGamal требуется несколько изменений спецификации создания туннелей [Tunnel-Creation]_.
...
Эти изменения определены в предложении 152 [Prop152]_.
...
Эти изменения определены в предложении 157 [Prop157]_.
...
ECIES-X25519-AEAD-Ratchet [Prop144]_,  теперь определено в [ECIES]_.
...
- Нет необходимости в "двойных ключах" для Менеджеров ключей сессий, как описано в [ECIES]_ для Destinations. У роутеров только один открытый ключ.


Выводы дизайна
```````````````````````

ECIES Router SKM не требует полного Ratchet SKM как указывается в [ECIES]_ для Destinations.
...
в [Prop152]_ для создания туннелей.
Он будет использовать тот же формат нагрузки, как указано в [ECIES]_ для Destinations.
Режим с нулевым статическим ключом (без связывания или сессии) IK, описанный в [ECIES]_, не будет использоваться.

Ответы на запросы шифруются ратчетным тегом, если запрашивается в поисковом запросе.
Это задокументировано в [Prop154]_, теперь указано в [I2NP]_.
...
описано в [ECIES]_ для Destinations.
...
имеют реализации ответов с тегами ECIES, как указано в [Prop152]_.
...
Идентификатор роутера и сертификат ключа: См. [Common]_.

Создание туннелей: См. [Prop152]_.

Новое сообщение о создании туннеля: См. [Prop157]_.


### Шифрование запросов

Шифрование запросов соответствует тому, что указано в [Tunnel-Creation-ECIES]_ и [Prop152]_,
...
как указано в [I2NP]_ и [Prop154]_. Ключ и тег используются для шифрования ответа.

Наборы тегов не создаются.
Схема с нулевым статическим ключом, указанная в
ECIES-X25519-AEAD-Ratchet [Prop144]_ и [ECIES]_, не используется.
...
Это стандарт [NOISE]_ для паттерна "N" с стандартным именем протокола.
Это соответствует тому, что указано в [Tunnel-Creation-ECIES]_ и [Prop152]_ для сообщений о создании туннелей.
...
Полезная нагрузка имеет тот же формат блока, что и определяется в [ECIES]_ и [Prop144]_.
...
как указано в [I2NP]_ и [Prop154]_.
...
Предложение 145 [Prop145]_ может не быть переписано для полной совместимости с
предложением 152 [Prop152]_.
...
к середине 2019 года в ответ на незавершенное предложение 145 [Prop145]_.
...
Реализуйте создание туннелей, как определено в предложении 152 [Prop152]_.
...
как определено в предложении 144 [Prop144]_.
...
Реализуйте и протестируйте новое сообщение о создании туннеля, как определено в предложении 157 [Prop157]_.
