...
date: 2012-12-17
author: "zzz"
description: "Версия 0.9.4 включает исправление ошибки, связанной с пропускной способностью сети, появившейся в 0.9.2, которая снижала производительность и надежность сети."
categories: ["release"]
---

0.9.4 включает исправление ошибки, связанной с пропускной способностью сети, появившейся в 0.9.2, которая снижала производительность и надёжность сети. Она также включает существенные изменения в системе обновлений внутри сети и добавляет возможность обновляться через торренты внутри сети.

Мы исправили несколько ошибок в реализации DHT в i2psnark, представленной в предыдущем релизе. Для тех, кто использует пароли консоли или HTTP-прокси, мы перешли на более безопасный метод аутентификации Digest и улучшили безопасность форм консоли.

Для тех из вас, кто уже использует сборки для разработчиков, ваш router должен автоматически обновиться до 0.9.4-0 с использованием нового внутррисетевого механизма торрентов. Для тех, кто использует 0.9.3-0, обновление будет происходить в обычном порядке через внутррисетевой HTTP, и мы предоставим дополнительную информацию при выпуске 0.9.5.
