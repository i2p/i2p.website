...
date: 2012-05-02
author: "zzz"
description: "La version 0.9 conclut plus d’un an et demi de travail sur la série 0.8.x, durant lesquels nous avons considérablement amélioré les performances et la sécurité du router, ainsi que l’évolutivité du réseau."
categories: ["release"]
---

La version 0.9 conclut plus d'un an et demi de travail sur la série 0.8.x, au cours de laquelle nous avons considérablement amélioré les performances et la sécurité du router, ainsi que la scalabilité du réseau.

Dans la version 0.9, nous migrons vers Jetty 6 pour la console et le serveur web de l'eepsite, et introduisons une page d'accueil simplifiée de la console du router. Cette version est compatible au niveau réseau avec les versions précédentes ; cependant, veuillez consulter ci-dessous des informations importantes concernant la compatibilité avec les eepsites, plugins et webapps existants. La mise à niveau est recommandée.

Les fichiers sont disponibles sur la [page de téléchargement](/downloads).

## Informations de mise à jour

Les mises à jour via le réseau et les nouvelles installations incluront Jetty 6, Tomcat et JSTL. Les mises à jour PPA (Ubuntu/Debian) auront de nouvelles dépendances pour ces paquets.

Si vous n’avez pas activé ou modifié les paramètres de votre eepsite, la mise à jour devrait automatiquement vous faire passer à Jetty 6. Si vous n’utilisez pas Jetty pour votre eepsite, la mise à jour ne l’affectera pas.

Si vous avez plus d’un eepsite Jetty ou si vous avez modifié le port local (7658 par défaut) ou modifié jetty.xml d’une autre manière, vous devez modifier manuellement les nouveaux fichiers de configuration de Jetty après la mise à jour, puis redémarrer.

Pour ceux qui utilisent des eepsites basés sur Jetty, nous vous recommandons de vérifier que la mise à niveau s'est bien déroulée et que les eepsites sont opérationnels après le redémarrage du router.
...
D'autres applications web non standard pour la console et les eepsites, qui ne sont pas empaquetées par l'équipe de développement d'I2P, peuvent, ou non, nécessiter des modifications.
