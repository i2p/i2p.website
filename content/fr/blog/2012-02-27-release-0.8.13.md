---
title: "Version 0.8.13"
date: 2012-02-27
author: "zzz"
description: "La version 0.8.13 contient plusieurs correctifs de bogues et quelques nouvelles fonctionnalités. Nous sommes heureux que la dernière version ait nettement amélioré les performances, et que le réseau fonctionne bien malgré la croissance rapide qui se poursuit."
categories: ["release"]
---

La version 0.8.13 contient plusieurs correctifs de bogues et quelques nouvelles fonctionnalités. Nous sommes heureux que la dernière version ait amélioré de façon significative les performances, et que le réseau fonctionne bien malgré une croissance rapide continue. La mise à jour est recommandée.

Les fichiers sont disponibles sur la [page de téléchargement](/downloads).

## DÉTAILS DE LA VERSION

Il s’agit probablement de la dernière version avant 0.9, au cours de laquelle nous migrerons vers Jetty 6 et introduirons une page d’accueil simplifiée de la console du router. Surveillez la section des actualités de votre la router console dans les prochaines semaines pour plus d’informations sur la prochaine version.

À partir de cette version, le router recherchera et installera les mises à jour de plugins peu après le redémarrage qui suit une mise à jour du router. Pour empêcher cela, ajoutez la configuration avancée `plugins.autoUpdate=false` avant de redémarrer. Il y a également un nouveau bouton update-all sur la page de configuration du client.

Les routers situés dans certains pays activeront désormais automatiquement le mode masqué pour une protection accrue. Cependant, le mode masqué peut avoir des performances ou une fiabilité moindres, et il est encore en cours de développement. Pour désactiver le mode masqué avant de redémarrer, ajoutez la configuration avancée `router.isHidden=false`. Pour le désactiver plus tard, sélectionnez *utiliser toutes les méthodes d'auto-détection* sous *Configuration IP* sur la page de configuration réseau. Pour la liste des pays, voir le fil sur zzz.i2p. Le seul pays de la liste qui compte plus d'un ou deux utilisateurs I2P est la Chine.

## Changements majeurs

- Check for and download plugin updates upon restarting after a router update
- Routers in certain countries will now automatically enable hidden mode for increased protection
- New Tunnel Wizard for creating tunnels
- A SIGHUP to the wrapper will now initiate a graceful shutdown

## Corrections de bogues

- Increase read timeout in HTTP Proxy
- Shutdown hooks will now run when router is shutdown externally (i2prouter stop), broken in 0.8.8
- If an external IP address is specified, bind only to that interface
- Fail tunnels we created when we can't contact the adjacent hop
- Prevent races when saving configuration
- For plugins, check min and max Jetty versions; check all version requirements at startup, not just at install
- Fix plugin startup when console is not on port 7657
- Only stop plugins if they are running
- Fix NPE when no network interfaces are present
- Fix eepget exit code on failure
- Improve inbound tunnel building when in hidden mode
- Publish our router info sooner after startup to facilitate inbound tunnel building
- Fix Streamr tunnel registration

## Autre

- Add icons to buttons in SusMail and SusiDNS
- Move wrapper PID, status, and log files from /tmp to ~/.i2p (Debian packages and new installs only)
- i2prouter graceful (Debian packages and new installs only)
- Increase number of floodfills
- Repack jars in installer to save a little space
- New translation infrastructure for i2prouter script (not enabled yet)
- New Czech translation (thanks Waseihou)
- German, Italian, Polish, Spanish, Swedish, Ukrainian translation updates

## Sommes de contrôle SHA256 :

```
648a552ee009e9648aba0666c05e2f71e442a714716659a0ca4f60aca87bda50  i2pinstall_0.8.13.exe
bdd510cc47b2cd78aa8d994e27694185c1f2deb48b049d61a93a795932ce5712  i2psource_0.8.13.tar.bz2
36683d906ac121fd28ac5600218aec943da154cb0230f274f0e2a7c6cc6fb8a5  i2pupdate_0.8.13.zip
81fa5256250fde2790153b83d2b01b6bc3b5ee7ea1b4d12232ce46f06ae736ef  i2pupdate.su2
2eb25974ebfeeeec59d8138e42d5663d97cc24b94f2c6cf77cfe6dc991acf0bb  i2pupdate.sud
```