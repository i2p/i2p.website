...
resistência do [SSU]_ a várias formas de identificação automatizada e ataques.
...
Assim como o [SSU]_, também fornece dois serviços adicionais:
...
- Suportar SSU 1 e 2 em uma única porta, detecção automática, e publicado como um único
  "transporte" (ou seja, [RouterAddress]_) no [NetDB]_.
...
- Corrigir perda de bits no DH devido ao formato Java [Ticket1112]_, e acelerar o DH
  mudando para X25519.
...
- Usar um handshake de três mensagens, de uma rodada de ida e volta, como no [NTCP2]_.
  Remover o atraso esperando por mensagens de dados que tornam [SSU]_
...
- Um transporte baseado em TLS (ou semelhante a HTTPS)... que seria [Prop104]_.
...
protocolo Station-To-Station (STS) [STS]_. Note que este protocolo é também a
base para o protocolo [SSU]_.
...
O seguinte é copiado do QUIC [RFC-9000]_.
...
em conformidade com [RFC-6437]_, a menos que a API local não permita
