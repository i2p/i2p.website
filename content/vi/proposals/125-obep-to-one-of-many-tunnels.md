...
một hash [LeaseSet]_, tương tự như khả năng hiện tại để chỉ định một hash
[RouterIdentity]_. Điều này sẽ dẫn đến một thông điệp nhỏ hơn và một LeaseSet có thể mới hơn. Tuy nhiên:
...
4. Chỉ định một LeaseSet tiết lộ cho OBEP [Destination]_ của thông điệp,
...
Người khởi tạo (OBGW) sẽ đặt một số (toàn bộ?) các [Leases]_ mục tiêu vào
các chỉ dẫn giao nhận [TUNNEL-DELIVERY]_ thay vì chỉ chọn một.
...
các cờ cho [TUNNEL-DELIVERY]_, mà chúng ta có thể tận dụng để thực hiện những tính năng này.
...
- Một kẻ tấn công kiểm soát OBEP và đang quét LeaseSets từ NetDB đã có thể xác định liệu
  ...
  [TunnelId]_ / [RouterIdentity]_. Tại tệ nhất, sự hiện diện của nhiều Lease trong TMDI
...
Các Chỉ dẫn Giao nhận Phân đoạn Đầu Tiên [TUNNEL-DELIVERY]_ sẽ được chỉnh sửa như sau:
...
trên phiên bản I2P mục tiêu [VERSIONS]_:

* OBGWs phải chọn các OBEP tương thích khi xây dựng các kênh outbound, dựa trên
  phiên bản I2P được quảng cáo trong [RouterInfo]_ của họ.
