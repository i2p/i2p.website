...
Xem [SPEC]_ để có được đặc tả chính thức.


## Tổng quan

Tài liệu này đề xuất thay đổi mã hóa tin nhắn xây dựng đường hầm
sử dụng các khối mật mã được giới thiệu bởi [ECIES-X25519]_.
Đây là một phần của đề xuất tổng thể
[Prop156]_ để chuyển đổi router từ ElGamal sang ECIES-X25519.
...
Để có tổng quan về tất cả các thay đổi cần thiết cho các router ECIES, hãy xem đề xuất 156 [Prop156]_.
...
- AES-256-CBC như trong [Cryptography]_
- Chức năng STREAM ChaCha20/Poly1305:
  ENCRYPT(k, n, plaintext, ad) và DECRYPT(k, n, ciphertext, ad) - như trong [NTCP2]_ [ECIES-X25519]_ và [RFC-7539]_
- Chức năng X25519 DH - như trong [NTCP2]_ và [ECIES-X25519]_
- HKDF(salt, ikm, info, n) - như trong [NTCP2]_ và [ECIES-X25519]_

Các chức năng Noise khác được định nghĩa ở nơi khác:

- MixHash(d) - như trong [NTCP2]_ và [ECIES-X25519]_
- MixKey(d) - như trong [NTCP2]_ và [ECIES-X25519]_
...
- Không thay đổi mã hóa "lớp" AES đường hầm, cho điều đó xem [Prop153]_
...
- Sử dụng các tùy chọn xây dựng đường hầm theo định nghĩa trong [Prop143]_, chỉ yêu cầu cho các thông điệp nhỏ
- Đường hầm hai chiều - cho điều đó xem [Prop119]_
- Các tin nhắn xây dựng đường hầm nhỏ hơn - cho điều đó xem [Prop157]_
...
[NOISE]_ (Bản sửa đổi 34, 2018-07-11).
...
- Chức năng DH: X25519
  X25519 DH với một chiều dài khóa 32 byte như đã được quy định trong [RFC-7748]_.

- Chức năng Mã hóa: ChaChaPoly
  AEAD_CHACHA20_POLY1305 như đã được quy định trong [RFC-7539]_ phần 2.8.
  12 byte nonce, với bốn byte đầu tiên đặt thành zero.
  Giống như trong [NTCP2]_.
...
Các thỏa thuận sử dụng mẫu [Noise]_.
...
Điều này cũng giống hệt với thông điệp đầu tiên (Yêu cầu Phiên) trong mẫu XK được sử dụng trong [NTCP2]_.
...
Mã hóa bất đối xứng của các bản ghi yêu cầu này hiện tại là ElGamal như được định nghĩa trong [Cryptography]_
...
Để tham khảo, đây là đặc tả hiện tại của bản ghi Yêu cầu Xây dựng Đường hầm cho các router ElGamal, lấy từ [I2NP]_.
Dữ liệu không mã hóa được đứng trước bởi một byte không phải zero và mã băm SHA-256 của dữ liệu trước khi mã hóa,
như được định nghĩa trong [Cryptography]_.
...
Trường cờ giống như đã định nghĩa trong [Tunnel-Creation]_ và chứa các giá trị sau::
...
Các tùy chọn xây dựng đường hầm là một cấu trúc Ánh xạ như được định nghĩa trong [Common]_.
...
Các tùy chọn trả lời xây dựng đường hầm là một cấu trúc Ánh xạ như được định nghĩa trong [Common]_.
Điều này dành cho sử dụng trong tương lai. Không có tùy chọn nào hiện đang được định nghĩa.
Nếu cấu trúc Ánh xạ trống, đây là hai byte 0x00 0x00.
...
như được định nghĩa trong [Tunnel-Creation]_ để tránh nhận diện dấu vân tay:
...
Trên đường trả lời, điểm cuối (người gửi) sẽ cần huỷ bỏ [Multiple-Encryption]_, bằng cách sử dụng mỗi khóa trả lời của bước nhảy.
...
Xem [Prop156]_ để biết chi tiết về các khóa ECIES tĩnh của router.
...
Đây là chuẩn [NOISE]_ cho mẫu "N" với một tên giao thức chuẩn.
...
sơ đồ được định nghĩa trong [Tunnel-Creation]_. Người tạo đường hầm ECIES sẽ sử dụng sơ đồ trên để mã hóa
...
Như được định nghĩa trong [Tunnel-Creation]_.
...
Như đã định nghĩa trong [Tunnel-Creation]_.
Không có thay đổi nào đối với mã hóa cho các bước nhảy ElGamal.
