...
Xem [SPEC](/docs/specs/implementation/) để có được đặc tả chính thức.


## Tổng quan

Tài liệu này đề xuất thay đổi mã hóa tin nhắn xây dựng đường hầm
sử dụng các khối mật mã được giới thiệu bởi [ECIES-X25519](/docs/specs/ecies/).
Đây là một phần của đề xuất tổng thể
[Prop156](/proposals/156-ecies-routers/) để chuyển đổi router từ ElGamal sang ECIES-X25519.
...
Để có tổng quan về tất cả các thay đổi cần thiết cho các router ECIES, hãy xem đề xuất 156 [Prop156](/proposals/156-ecies-routers/).
...
thực hiện sau - xem [Prop157](/proposals/157-new-tbm/).
...
- Chức năng X25519 DH - như trong [NTCP2](/docs/specs/ntcp2/) và [ECIES-X25519](/docs/specs/ecies/)
...
- Không thay đổi mã hóa "lớp" AES đường hầm, cho điều đó xem [Prop153](/proposals/153-chacha20-layer-encryption/)
...
- Sử dụng các tùy chọn xây dựng đường hầm theo định nghĩa trong [Prop143](/proposals/143-build-message-options/), chỉ yêu cầu cho các thông điệp nhỏ
- Đường hầm hai chiều - cho điều đó xem [Prop119](/proposals/119-bidirectional-tunnels/)
- Các tin nhắn xây dựng đường hầm nhỏ hơn - cho điều đó xem [Prop157](/proposals/157-new-tbm/)
...
- Chức năng Mã hóa: ChaChaPoly
  ...
  Giống như trong [NTCP2](/docs/specs/ntcp2/).
...
Điều này cũng giống hệt với thông điệp đầu tiên (Yêu cầu Phiên) trong mẫu XK được sử dụng trong [NTCP2](/docs/specs/ntcp2/).
...
Mã hóa bất đối xứng của các bản ghi yêu cầu này hiện tại là ElGamal như được định nghĩa trong [Cryptography](/docs/specs/cryptography/)
...
Để tham khảo, đây là đặc tả hiện tại của bản ghi Yêu cầu Xây dựng Đường hầm cho các router ElGamal, lấy từ [I2NP](/docs/specs/i2np/).
Dữ liệu không mã hóa được đứng trước bởi một byte không phải zero và mã băm SHA-256 của dữ liệu trước khi mã hóa,
như được định nghĩa trong [Cryptography](/docs/specs/cryptography/).
...
Trường cờ giống như đã định nghĩa trong [Tunnel-Creation](/docs/specs/tunnel-creation/) và chứa các giá trị sau::
...
Các tùy chọn xây dựng đường hầm là một cấu trúc Ánh xạ như được định nghĩa trong [Common](/docs/specs/common-structures/).
...
Các tùy chọn trả lời xây dựng đường hầm là một cấu trúc Ánh xạ như được định nghĩa trong [Common](/docs/specs/common-structures/).
Điều này dành cho sử dụng trong tương lai. Không có tùy chọn nào hiện đang được định nghĩa.
Nếu cấu trúc Ánh xạ trống, đây là hai byte 0x00 0x00.
...
như được định nghĩa trong [Tunnel-Creation](/docs/specs/tunnel-creation/) để tránh nhận diện dấu vân tay:
...
Xem [Prop156](/proposals/156-ecies-routers/) để biết chi tiết về các khóa ECIES tĩnh của router.
...
sơ đồ được định nghĩa trong [Tunnel-Creation](/docs/specs/tunnel-creation/). Người tạo đường hầm ECIES sẽ sử dụng sơ đồ trên để mã hóa
...
Như được định nghĩa trong [Tunnel-Creation](/docs/specs/tunnel-creation/).
...
Như đã định nghĩa trong [Tunnel-Creation](/docs/specs/tunnel-creation/).
Không có thay đổi nào đối với mã hóa cho các bước nhảy ElGamal.
...
Xem [Prop156](/proposals/156-ecies-routers/).
