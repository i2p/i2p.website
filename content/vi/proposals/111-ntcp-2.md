...
Đề xuất này mô tả một giao thức thỏa thuận khóa được xác thực nhằm cải thiện khả năng chống lại các hình thức tấn công và xác định tự động khác nhau của [NTCP]_.
...
Các tác giả thừa nhận rằng thực hành bảo mật tiêu chuẩn sẽ gợi ý sử dụng một giao thức hiện có như TLS, nhưng đây là [Prop104]_ và nó có những vấn đề riêng của nó. Ở bất kỳ nơi nào thích hợp, các đoạn "công việc trong tương lai" đã được thêm vào để chỉ ra các tính năng thiếu hoặc các chủ đề thảo luận.


## Mục tiêu thiết kế

- Hỗ trợ NTCP 1 và 2 trên một cổng duy nhất, tự động phát hiện và công khai như một "transport" đơn lẻ (tức là [RouterAddress]_) trong [NetDB]_.
...
- Sửa mất bits trong DH do định dạng Java [Ticket1112]_, có thể (và có lẽ) bằng cách chuyển sang X25519.
...
- Tiếp tục sử dụng các loại chữ ký có biến đổi, biến độ dài (từ khóa ký [RouterIdentity]_ đã được công khai) như một phần của xác thực. Dựa vào một khóa công khai tĩnh được công khai trong RouterInfo như là phần khác của xác thực.
...
- Nếu có thể, giảm bắt tay 4 thông điệp, hai vòng lặp xuống 3 thông điệp, một vòng lặp, như trong [SSU]_. Điều này sẽ yêu cầu chuyển chữ ký của Bob trong thông điệp 4 sang thông điệp 2. Nghiên cứu lý do cho 4 thông điệp trong các email/trạng thái/cuộc gặp mười năm trước.
...
- Khả năng chống lại DPI một cách đầy đủ... đó sẽ là các trao đổi vận hành cắm được, [Prop109]_.

- Một vận chuyển dựa trên TLS (hoặc mô phỏng HTTPS)... đó sẽ là [Prop104]_.
...
Cả Alice và Bob đều sở hữu một cặp khóa tĩnh, chứa trong [RouterIdentity]_ của họ.
...
Đề xuất hiện tại cố gắng cung cấp tất cả năm yêu cầu này dựa trên giao thức Station-To-Station (STS) [STS]_. Lưu ý rằng giao thức này cũng là cơ sở cho giao thức [SSU]_.
...
[NOISE]_ (Phiên bản 33, 2017-10-04).
Noise có các thuộc tính tương tự như giao thức Station-To-Station
[STS]_, là cơ sở cho giao thức [SSU]_. Theo cách nói của Noise, Alice
...
- Chức năng DH: X25519
  X25519 DH với độ dài khóa 32 byte được chỉ định trong [RFC-7748]_.

- Chức năng mã: ChaChaPoly
  AEAD_CHACHA20_POLY1305 được chỉ định trong [RFC-7539]_ phần 2.8.
...
Đối với ChaCha20, những gì được mô tả ở đây tương ứng với [RFC-7539]_, cũng được sử dụng tương tự trong TLS [RFC-7905]_.
...
KDF tạo ra một mã khóa pha bắt tay từ kết quả DH, sử dụng HMAC-SHA256(key, data) như được định nghĩa trong [RFC-2104]_.
...
Đây là mẫu thông điệp "e":
  ...
  Định nghĩa HMAC-SHA256(khóa, dữ liệu) như trong [RFC-2104]_
