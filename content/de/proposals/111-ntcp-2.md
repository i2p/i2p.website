...
Dieser Vorschlag beschreibt ein authentifiziertes Schlüsselvereinbarungsprotokoll zur Verbesserung des Widerstands von [NTCP]_ gegen verschiedene Formen automatisierter Identifikation und Angriffe.
...
Die [NTCP]_ Daten werden nach der ersten Nachricht verschlüsselt (und die erste Nachricht erscheint als zufällige Daten), was eine Protokollidentifikation durch "Payload-Analyse" verhindert. Es ist weiterhin anfällig für Protokollidentifikation durch "Flow-Analyse". Das liegt daran, dass die ersten 4 Nachrichten (also der Handshake) eine feste Länge haben (288, 304, 448 und 48 Bytes).

Indem zufällige Mengen an Zufallsdaten zu jeder der Nachrichten hinzugefügt werden, können wir es erheblich erschweren.

Die Autoren erkennen an, dass Standard-Sicherheitspraktiken vorschlagen würden, ein vorhandenes Protokoll wie TLS zu verwenden, aber dies ist [Prop104]_ und es hat seine eigenen Probleme. Wo immer geeignet, wurden "Zukunftsarbeit"-Absätze hinzugefügt, um fehlende Funktionen oder Diskussionsthemen anzuzeigen.


## Designziele

- Unterstützung von NTCP 1 und 2 auf einem einzigen Port, automatische Erkennung und Veröffentlichung als ein einziger "Transport" (d. h. [RouterAddress]_) im [NetDB]_.
...
- Verlust von Bits in DH aufgrund des Java-Formats [Ticket1112]_ beheben, möglicherweise (wahrscheinlich?) durch Umstellung auf X25519.
...
- Wenn möglich, reduzieren Sie den 4-Nachrichten-, zwei-Roundtrip-Handshake auf einen 3-Nachrichten-, ein-Roundtrip-Handshake, wie in [SSU]_. Dies würde erfordern, Bobs Signatur in Nachricht 4 in Nachricht 2 zu verschieben. Untersuchen Sie den Grund für 4 Nachrichten in den zehn Jahre alten E-Mail-/Status-/Meeting-Archiven.
...
- Abhör-Festigkeit... das wären anschließbare Transporte, [Prop109]_.

- Ein TLS-basierter (oder HTTPS-ähnlicher) Transport... das wäre [Prop104]_.
...
Alice und Bob sind beide im Besitz eines statischen Schlüsselpaares, das in ihrem [RouterIdentity]_ enthalten ist.
...
Der vorliegende Vorschlag versucht, alle fünf Anforderungen auf der Basis des Station-To-Station (STS) Protokolls [STS]_ zu erfüllen. Beachten Sie, dass dieses Protokoll auch die Grundlage für das [SSU]_ Protokoll ist.
...
- Berücksichtigen Sie das Verhalten des Protokolls, wenn Pakete von einem Angreifer gelöscht oder umgeordnet werden. Aktuell interessante Arbeiten in diesem Bereich finden sich in [IACR-1150]_.
...
Dieser Vorschlag erfüllt die Anforderungen basierend auf dem Noise-Protokollrahmen [NOISE]_ (Revision 33, 2017-10-04). Noise hat ähnliche Eigenschaften wie das Station-To-Station-Protokoll [STS]_, das die Basis für das [SSU]_ ist. In der Noise-Terminologie ist Alice der Initiator und Bob der Empfänger.
...
Dieser Vorschlag definiert die folgenden Erweiterungen von Noise_XK_25519_ChaChaPoly_SHA256. Diese folgen im Allgemeinen den Richtlinien in [NOISE]_ Abschnitt 13.
...
Für ChaCha20, was hier beschrieben wird, entspricht [RFC-7539]_, das ähnlich auch in TLS [RFC-7905]_ verwendet wird.
...
Die KDF erzeugt einen Chiffreschlüssel k für die Handshake-Phase aus dem DH-Ergebnis, unter Verwendung von HMAC-SHA256(key, data) wie definiert in [RFC-2104]_.
...
Dies ist das "e"-Nachrichtenmuster:
  ...
  Definiere HMAC-SHA256(key, data) wie in [RFC-2104]_
...
unter Verwendung von HMAC-SHA256(key, data) wie in [RFC-2104]_ definiert.
