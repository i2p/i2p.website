...
resistance of [SSU](/docs/specs/ssu2/) to various forms of automated identification and attacks.
...
Like [SSU](/docs/specs/ssu2/), it also provides two additional services:
...
- Support SSU 1 and 2 on a single port, auto-detect, and published as a single
  "transport" (i.e. RouterAddress) in the [NetDB](/docs/how/network-database/).
...
- Fix loss of bits in DH due to Java format [Ticket1112](https://trac.i2p2.i2p/ticket/1112), and speed up the DH
...
- Use a 3-message, one-round-trip handshake, as in [NTCP2](/docs/specs/ntcp2/).
  Remove the delay waiting for data messages that makes
  [SSU](/docs/specs/ssu2/) effectively a two-round-trip handshake.
...
- A TLS-based (or HTTPS-lookalike) transport... that would be [Proposal 104](/proposals/104-tls-transport/).
...
basis for the [SSU](/docs/specs/ssu2/) protocol.
...
Following is copied from QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000).
...
(see Section 5.2 of [QUIC-TLS](https://datatracker.ietf.org/doc/html/rfc9001)).  Alternatively, the value is echoed
by the server in Version Negotiation packets (Section 6) or included
in the Integrity Tag in Retry packets (Section 5.8 of [QUIC-TLS](https://datatracker.ietf.org/doc/html/rfc9001)).
...
(PTO); see Section 6.2 of [QUIC-RECOVERY](https://datatracker.ietf.org/doc/html/rfc9002).  Specifically, the client
...
(using kInitialRtt, as defined in [QUIC-RECOVERY](https://datatracker.ietf.org/doc/html/rfc9002)) is RECOMMENDED.
...
B.3 of [QUIC-RECOVERY](https://datatracker.ietf.org/doc/html/rfc9002)) unless the only change in the peer's address
...
(as described in [QUIC-RECOVERY](https://datatracker.ietf.org/doc/html/rfc9002)) could be adequate.  For instance,
...
[QUIC-RECOVERY](https://datatracker.ietf.org/doc/html/rfc9002) and MUST NOT be more aggressive.
...
alternative services [ALTSVC](https://datatracker.ietf.org/doc/html/rfc7838).  Information that might allow correct
...
connection ID, source UDP port, or IP address (see [RFC8981](https://datatracker.ietf.org/doc/html/rfc8981)) when
...
in compliance with [RFC 6437](https://tools.ietf.org/html/rfc6437), unless the local API does not allow
...
QUIC assumes the threat model described in [SEC-CONS](https://datatracker.ietf.org/doc/html/rfc9000#section-21) and provides
...
[SEC-CONS](https://datatracker.ietf.org/doc/html/rfc9000#section-21) in that an off-path attacker is able to observe packets.
...
the cryptographic properties described in Appendix E.1 of [TLS13](https://datatracker.ietf.org/doc/html/rfc8446).
...
specific keying material; see [QUIC-TLS](https://datatracker.ietf.org/doc/html/rfc9001) for more details.  This
...
most part, the cryptographic handshake protocol [QUIC-TLS](https://datatracker.ietf.org/doc/html/rfc9001) is
...
For example, cross-site request forgery [CSRF](https://owasp.org/www-community/attacks/csrf) exploits on the Web
cause a client to issue requests that include authorization cookies
[COOKIE](https://datatracker.ietf.org/doc/html/rfc6265), allowing one site access to information and actions that
...
filtering [BCP38](https://datatracker.ietf.org/doc/html/rfc2827) and also have inadequately secured UDP endpoints.
...
Initial packet protection (Section 5.2 of [QUIC-TLS](https://datatracker.ietf.org/doc/html/rfc9001)) makes it
...
the network level -- in particular, ingress filtering [BCP38](https://datatracker.ietf.org/doc/html/rfc2827) -- are
...
ingress filtering [BCP38](https://datatracker.ietf.org/doc/html/rfc2827) is also effective.
...
address [RFC4291](https://datatracker.ietf.org/doc/html/rfc4291) or an address in a private-use range [RFC1918](https://datatracker.ietf.org/doc/html/rfc1918) from
a global, unique-local [RFC4193](https://datatracker.ietf.org/doc/html/rfc4193), or non-private address as a
...
The attacks commonly known as Slowloris [SLOWLORIS](https://owasp.org/www-community/attacks/Slowloris_Attack) try to keep many
...
IP header to influence the sender's rate.  [RFC3168](https://datatracker.ietf.org/doc/html/rfc3168) discusses
...
* Threat models: From NTCP2 [NTCP2](/docs/specs/ntcp2/), with significant additional threats
  relevant to UDP transport as analyzed by QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001).

* Cryptographic choices: From [NTCP2](/docs/specs/ntcp2/).

* Handshake: Noise XK from [NTCP2](/docs/specs/ntcp2/) and [NOISE](https://noiseprotocol.org/noise.html). Significant simplifications
  to NTCP2 are possible due to the encapsulation (inherent message boundaries)
  provided by UDP.

* Handshake ephemeral key obfuscation: Adapted from [NTCP2](/docs/specs/ntcp2/)
  but using ChaCha20 from [ECIES](/docs/specs/ecies/) instead of AES.

* Packet headers: Adapted from WireGuard [WireGuard](https://www.wireguard.com/protocol/) and QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001).

* Packet header obfuscation: Adapted from [NTCP2](/docs/specs/ntcp2/)
  but using ChaCha20 from [ECIES](/docs/specs/ecies/) instead of AES.

* Packet header protection: Adapted from QUIC [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001) and [Nonces](https://eprint.iacr.org/2019/624.pdf)

* Headers used as AEAD associated data as in [ECIES](/docs/specs/ecies/).

* Packet numbering: Adapted from WireGuard [WireGuard](https://www.wireguard.com/protocol/) and QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001).

* Messages: Adapted from [SSU](/docs/specs/ssu2/)

* I2NP Fragmentation: Adapted from [SSU](/docs/specs/ssu2/)

* Relay and Peer Testing: Adapted from [SSU](/docs/specs/ssu2/)
...
* Acks, nacks: Adapted from QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000).
...
Station-To-Station (STS) protocol, which is the basis for the [SSU](/docs/specs/ssu2/) protocol.  In Noise parlance, Alice
...
is not allowed for several reasons. For background see QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) section 12.3.
...
- The packet number of a data phase packet is an AEAD nonce and is critical data.
  ...
  with knowledge of the introduction key found in the netdb.
  See [Nonces](https://eprint.iacr.org/2019/624.pdf).
...
using ChaCha20, similar to QUIC [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001) and [Nonces](https://eprint.iacr.org/2019/624.pdf).
...
Unlike the QUIC [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001) header protection scheme,
ALL parts of all headers, including destination and source connection IDs,
are encrypted.
QUIC [RFC 9001](https://datatracker.ietf.org/doc/html/rfc9001) and [Nonces](https://eprint.iacr.org/2019/624.pdf) are primarily focused on encrypting
...
For ChaCha20, what is described here corresponds to [RFC 7539](https://tools.ietf.org/html/rfc7539), which is also
...
using HMAC-SHA256(key, data) as defined in [RFC 2104](https://tools.ietf.org/html/rfc2104).
...
This uses the same block format as defined in the [NTCP2](/docs/specs/ntcp2/) and [ECIES](/docs/specs/ecies/) specifications.
Individual block types are defined differently.
The equivalent term in QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) is "frames".
...
This format is identical to that in [NTCP2](/docs/specs/ntcp2/) and [ECIES](/docs/specs/ecies/),
...
as in [NTCP2](/docs/specs/ntcp2/) (type, message id, short expiration).
...
lower than that.
See QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) section 13.2.3 for more background.
...
- The padding length is either to be decided on a per-message basis and
  ...
  of [NTCP2](/docs/specs/ntcp2/) provides more information on the topic.
...
General guidance may be found in [RFC 9002](https://datatracker.ietf.org/doc/html/rfc9002).
See also [RFC 6298](https://tools.ietf.org/html/rfc6298) for guidance on retransmission timers.
...
packet count, following the guidance in TCP RFCs and QUIC [RFC 9002](https://datatracker.ietf.org/doc/html/rfc9002).
...
using return path capacity; see Section 3 of [RFC 3449](https://tools.ietf.org/html/rfc3449).
...
consistent with recommendations for TCP endpoint behavior [RFC 5681](https://tools.ietf.org/html/rfc5681).
...
This process is a simplified version of that defined in QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000).
...
### Threat Model
(Adapted from QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000))
...
QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) specified changing connection IDs when changing network paths.
...
which is the same as specified in [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000).
See below.
...
Recent studies [PMTU](https://aura.abdn.ac.uk/bitstream/handle/2164/11693/tma2018_paper57.pdf) suggest that a minimum for IPv4 of 1200 or more would work
for more than 99% of connections. QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) requires a minimum IP
packet size of 1280 bytes.

quote [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000):
...
[Proposal 158](/proposals/158-ipv6-transport-enhancements/) and supported as of 0.9.50.
...
including changes in [Proposal 158](/proposals/158-ipv6-transport-enhancements/) supported as of API 0.9.50:
...
* [Nonces](https://eprint.iacr.org/2019/624.pdf)
* [NTCP](/docs/legacy/ntcp/)
...
* [Prop158](/proposals/158-ipv6-transport-enhancements/)
