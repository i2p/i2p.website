...
description: "Historical discussion about NTCP vs SSU transport protocols from March 2007"
...
### "NTCP Considered Harmful" Analysis by zzz {#harmful}
...
2. **1B)** Alternative to 1A), not so easy -
...
3. **2)** Bounding SSU even further (reducing maximum retransmissions from the current
   10) is probably wise to reduce the chance of collapse.

4. **3)** We need further study on the benefits vs. harm of a semi-reliable protocol
...
5. **4)** The priority sending code in .28 is only for NTCP. So far my testing hasn't
...
6. **5)** The new streaming lib max timeout of 45s is probably still too low.
   The TCP RFC says 60s. It probably shouldn't be shorter than the underlying NTCP max timeout (presumably 60s).


### Response by jrandom {#jrandom-response}
...
> *1B) Alternative to 1A), not so easy -
> If we can mark traffic without adversely affecting our anonymity goals, we
...
> *2) Bounding SSU even further (reducing maximum retransmissions from the current
> 10) is probably wise to reduce the chance of collapse.*
...
> *3) We need further study on the benefits vs. harm of a semi-reliable protocol
> underneath the streaming lib. Are retransmissions over a single hop beneficial
> and a big win or are they worse than useless?
> We could do a new SUU (secure unreliable UDP) but probably not worth it. We
> could perhaps add a no-ACK-required message type in SSU if we don't want any
...
> *4) The priority sending code in .28 is only for NTCP. So far my testing hasn't
> shown much use for SSU priority as the messages don't queue up long enough for
> priorities to do any good. But more testing needed.*
...
> *5) The new streaming lib max timeout of 45s is probably still too low. The TCP RFC
...
### Question answered by jrandom {#jrandom-followup}
...
### Response by foofighter {#foofighter}
...
### Response by zzz (to foofighter) {#zzz-foofighter}
...
## Results of the Discussion {#results}
