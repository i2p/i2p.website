---
title: "SAM V2 Specification"
description: "Legacy Simple Anonymous Messaging protocol version 2 (deprecated)"
slug: "samv2"
lastUpdated: "2025-03"
accurateFor: "0.9.20"
---
...
Specified below is version 2 of a simple client protocol for interacting with I2P.
...
client\<--\>SAM socket.
For multiple sockets, see [SAM V3](/docs/api/samv3).
...
- `BUFFER_FULL` - SAM's buffer has 32 or more KB of data to send, and subsequent SEND requests will fail
- `READY` - SAM's buffer is not full, and the next SEND request is granted to be successful
...
**\*\*\*** If the SAM bridge replied with BUFFER_FULL, it will send another message as soon as its buffer is available again:
...
- `NONE` - the SAM bridge will keep on listening and deliver incoming data (same behavior as in version 1.0)
- an integer (less than 2^64) - the number of received bytes after which the SAM bridge will stop listening on the incoming stream. Whenever the client is ready to accept more bytes from the stream, it has to send such a message again, with a greater $limit.
