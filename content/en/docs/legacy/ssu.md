...
description: "Original UDP transport protocol specification (deprecated, replaced by SSU2)"
slug: "ssu"
aliases:
  - /docs/specs/ssu-spec/
category: "Transports"
...
DEPRECATED - SSU has been replaced by SSU2. SSU support was removed from
i2pd in release 2.44.0 (API 0.9.56) 2022-11. SSU support was removed
from Java I2P in release 2.4.0 (API 0.9.61) 2023-12.

See the [SSU overview](/docs/transport/ssu/) for more information.
...
The exclusive OR of ((netid - 2) << 8) is used to quickly identify
...
the chances of the minimal representation being less than 64 bytes is
vanishingly small.
...
using the [SigningPublicKey](/docs/specs/common-structures/#signingpublickey) from the [RouterIdentity](/docs/specs/common-structures/#routeridentity) which is distributed
...
[KeyCertificate](/docs/specs/common-structures/#key-certificates) in Bob's [RouterIdentity](/docs/specs/common-structures/#routeridentity).

Both introduction keys and session keys are 32 bytes, and are defined by the
Common structures specification [SessionKey](/docs/specs/common-structures/#sessionkey). The key used for the MAC and
...
While most of I2P uses 8-byte [Date](/docs/specs/common-structures/#date) timestamps with millisecond resolution,
...
- For the signature, Bob must use his external port, as that what Alice will
  ...
  will fail.
- See the [Keys](#keys) section above for details on signatures. Alice already has
...
- Through release 0.9.15, the signature was always a 40 byte DSA signature and
  ...
  length are implied by the type of the [SigningPublicKey](/docs/specs/common-structures/#signingpublickey) in Bob's
  [RouterIdentity](/docs/specs/common-structures/#routeridentity). The padding is as necessary to a multiple of 16 bytes.
...
This is the response to a [SessionCreated](#sessioncreated) message and the last step in
...
or 480 bytes (with DSA-SHA1 signature) (before non-mod-16 padding)
...
- In the current implementation, the maximum fragment size is 512 bytes. This
  ...
  two fragments.
- The typical [RouterIdentity](/docs/specs/common-structures/#routeridentity) is 387 bytes, so no fragmentation is ever
...
- Through release 0.9.15, the signature was always a 40 byte DSA signature. As
  ...
  the [SigningPublicKey](/docs/specs/common-structures/#signingpublickey) in Alice's [RouterIdentity](/docs/specs/common-structures/#routeridentity). The padding is as
...
- The IP address is only included if it is different than the packet's
...
This is the response to a [RelayRequest](#relayrequest) and is sent from Bob to Alice.
...
- For IPv4, Alice's IP address is always 4 bytes, because Alice is trying to connect to Charlie via IPv4.
  As of release 0.9.50, IPv6 is supported, and Alice's IP address may be 16 bytes.
...
**Data:** 1 byte flags (see below); if explicit ACKs are included: a 1 byte number of ACKs, that many 4 byte MessageIds being fully ACKed; if ACK bitfields are included: a 1 byte number of ACK bitfields, that many 4 byte MessageIds + a 1 or more byte ACK bitfield (see notes); If extended data included: 1 byte data size, that many bytes of extended data (currently uninterpreted); 1 byte number of fragments (can be zero); If nonzero, that many message fragments.
...
The bitfield uses the 7 low bits of each byte, with the high bit specifying whether an
additional bitfield byte follows it (1 = true, 0 = the current bitfield byte is the last).
These sequence of 7 bit arrays represent whether a fragment has been received - if a bit is 1,
the fragment has been received. To clarify, assuming fragments 0, 2, 5, and 9 have been received,
...
See [SSU Peer Testing](/docs/transport/ssu/#peerTesting) for details.
...
4. When sent from Bob to Alice: Alice/Bob sessionKey (or for Bob prior to 0.9.52, Alice's introKey)
5. When sent from Charlie to Alice: Alice's introKey, as received in the PeerTest message from Bob
6. When sent from Alice to Charlie: Charlie's introKey, as received in the PeerTest message from Charlie
...
4. When sent from Bob to Alice: Alice's introKey, as received in the PeerTest message from Alice
5. When sent from Charlie to Alice: Alice's introKey, as received in the PeerTest message from Bob
6. When sent from Alice to Charlie: Charlie's introKey, as received in the PeerTest message from Charlie
