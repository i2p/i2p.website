---
title: "Low-level Cryptography Specification"
description: "Low-level details of the cryptographic algorithms used in I2P"
slug: "cryptography"
category: "Design"
lastUpdated: "2025-05"
accurateFor: "0.9.66"
...
> **Note:** This document is mostly obsolete. See the following documents for current specifications:
...
For the details of X25519 usage see [NTCP2](/docs/specs/ntcp2) and [ECIES](/docs/specs/ecies).
...
- For end-to-end (destination-to-destination) encryption as a part of ElGamal/AES+SessionTag using the encryption key in the LeaseSet
- For encryption of some netDb stores and queries sent to floodfill routers as a part of ElGamal/AES+SessionTag (destination-to-router or router-to-router).

We use common primes for 2048 ElGamal encryption and decryption, as given by IETF [RFC-3526](http://tools.ietf.org/html/rfc3526). We currently only use ElGamal to encrypt the IV and session key in a single block, followed by the AES encrypted payload using that key and IV.
...
See the ElGamal code [ElGamalEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/ElGamalEngine.java).

The shared prime is the Oakley prime for 2048 bit keys [RFC-3526-S3](http://tools.ietf.org/html/rfc3526#section-3):
...
Prior to release 0.9.8, all routers used the short exponent. As of release 0.9.8, 64-bit x86 routers use a full 2048-bit exponent. All routers now use the full exponent except for a small number of routers on very slow hardware, who continue to use the short exponent due to concerns about processor load. The transition to a longer exponent for these platforms is a topic for further study.
...
The original symmetric encryption algorithm in I2P is AES. The newer algorithm, used in several places, is Authenticated Encryption with Associated Data (AEAD) ChaCha20/Poly1305.
...
For the details of ChaCha20/Poly1305 usage see [NTCP2](/docs/specs/ntcp2) and [ECIES](/docs/specs/ecies).
...
- For encryption of some netDb stores and queries sent to floodfill routers as a part of ElGamal/AES+SessionTag (destination-to-router or router-to-router).
- For encryption of periodic tunnel test messages sent from the router to itself, through its own tunnels.

We use AES with 256 bit keys and 128 bit blocks in CBC mode. The padding used is specified in IETF [RFC-2313](http://tools.ietf.org/html/rfc2313) (PKCS#5 1.5, section 8.1 (for block type 02)). In this case, padding exists of pseudorandomly generated octets to match 16 byte blocks. Specifically, see the CBC code [CryptixAESEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/CryptixAESEngine.java) and the Cryptix AES implementation [CryptixRijndael_Algorithm](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/CryptixRijndael_Algorithm.java), as well as the padding, found in the ElGamalAESEngine.getPadding function [ElGamalAESEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/ElGamalAESEngine.java).
...
Signatures are generated and verified with 1024 bit [DSA](http://en.wikipedia.org/wiki/Digital_Signature_Algorithm) (L=1024, N=160), as implemented in [DSAEngine](https://github.com/i2p/i2p.i2p/tree/master/core/java/src/net/i2p/crypto/DSAEngine.java). DSA was chosen because it is much faster for signatures than ElGamal.
...
[NIST-800-57](http://csrc.nist.gov/publications/nistpubs/800-57/sp800-57-Part1-revised2_Mar08-2007.pdf) recommends a minimum of (L=2048, N=224) for usage beyond 2010. This may be mitigated somewhat by the "cryptoperiod", or lifespan of a given key.
...
Standard EdDSA using curve 25519 and standard 512-bit SHA-2 hashes, but with different private keys, and minor modifications to signing. For encrypted leasesets. See [EncryptedLeaseSet](/docs/specs/encryptedleaseset) and [Red25519](/docs/specs/red25519) for details.
...
All transports provide perfect forward secrecy [PFS](http://en.wikipedia.org/wiki/Perfect_forward_secrecy) on the transport links.

### NTCP2 connections {#tcp}

NTCP2 connections use X25519 Diffie-Hellman and ChaCha20/Poly1305 authenticated encryption, and the Noise protocol framework [Noise](https://noiseprotocol.org/noise.html).

See the NTCP2 specification [NTCP2](/docs/specs/ntcp2) for details and references.
...
NTCP connections were negotiated with a 2048 Diffie-Hellman implementation, using the router's identity to proceed with a station to station agreement, followed by some encrypted protocol specific fields, with all subsequent data encrypted with AES (as above). The primary reason to do the DH negotiation instead of using ElGamalAES+SessionTag is that it provides '(perfect) forward secrecy' [PFS](http://en.wikipedia.org/wiki/Perfect_forward_secrecy), while ElGamalAES+SessionTag does not.

## References

- [BENCHMARKS](https://web.archive.org/web/20080423000000*/http://www.eskimo.com/~weidai/benchmarks.html) - Crypto++ benchmarks, originally at http://www.eskimo.com/~weidai/benchmarks.html (now dead), rescued from http://www.archive.org/, dated Apr 23, 2008.
