...
An I2P configuration file is formatted as specified in Java [Properties](http://docs.oracle.com/javase/1.5.0/docs/api/java/util/Properties.html#load%28java.io.InputStream%29) with the following exceptions:
...
- Lines without `=` are ignored. As of release 0.9.10, keys with a value of "" are supported.
...
storeProps() [DATAHELPER](http://docs.i2p-projekt.de/javadoc/net/i2p/data/DataHelper.html). Note that the file format is significantly different than the serialized format for I2P protocols specified in [Mapping](/docs/specs/common-structures/#type-mapping).
...
: The constructor or main() method in this class will be run, depending on whether the client is managed or unmanaged. See below for details.

**name**
: Name to be displayed on console.

**args**
: Arguments to the main class, separated by spaces or tabs. Arguments containing spaces or tabs may be quoted with `'` or `"`
...
See the [plugin specification](/docs/specs/plugin). Note that
plugins may also contain clients.config, i2ptunnel.config, and
webapps.config files.
...
Note: "tunnel.N.option.i2cp.*" options, while appearing to be I2CP
options, are implemented in i2ptunnel, and are NOT supported via other
interfaces or APIs such as I2CP or SAM.
