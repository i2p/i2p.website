...
- ElG: ElGamal
- ENCRYPT(k, n, payload, ad): [ECIES]_에서 정의된 바와 같이
- LS: Leaseset
- 조회: I2NP DLM
- 응답: I2NP DSM 또는 DSRM
...
[ECIES]_ 목적을 사용할 때, 제안 144의 의도는 더 이상 목적지가 32바이트 태그 및 AES 복호화를 지원하지 않는 것입니다.
...
- 새로운 DLM 형식은 ECIES 암호화 응답을 지정하기 위해 플래그 필드에 비트를 추가할 것입니다. ECIES 암호화 응답은 [ECIES]_ 기존 세션 메시지 형식을 사용할 것이며, 태그를 앞에 붙이고 ChaCha/Poly 페이로드 및 MAC을 포함합니다.
...
[I2NP]_ DLM (DatabaseLookup) 사양에서 다음의 변경 사항을 적용합니다.
...
응답은 [ECIES]_에서 정의된 대로 ECIES 기존 세션 메시지입니다.
...
ECIES 라우터는 0.9.48에 도입되었습니다, [Prop156]_를 참조하십시오.
0.9.49부터 ECIES 목적지와 라우터는 "ECIES에서 ElG" 섹션의 형식을 동일하게 사용할 수 있으며, 요청에 포함된 응답 키를 사용합니다.
조회는 요청자가 익명이므로 [ECIES]_에서 "일회성 형식"을 사용합니다.
...
// Alice의 X25519 임시 키
  ...
  tag :: [ECIES]_에서 RATCHET_TAG()로 생성된 8바이트 태그

  k :: [ECIES]_에서 RATCHET_KEY()로 생성된 32바이트 키
