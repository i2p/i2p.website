...
이 제안서의 위임 부분에 대한 대안은 [LeaseSet]_ 해시를 통해 메시지를 보내는 것이며, 이는 기존에 타겟 [RouterIdentity]_ 해시를 지정할 수 있는 능력과 유사합니다. 이것은 더 작은 메시지와 잠재적으로 더 새로운 LeaseSet을 생성하지만:
...
발신자(OBGW)는 타겟 [Leases]의 일부(또는 전부)를 전달 지침 [TUNNEL-DELIVERY]_에 배치할 것입니다.

OBEP는 그 중 하나를 선택하여 전달합니다. OBEP는 이미 연결되었거나 알고 있는 터널을 선택합니다. 이것은 OBEP-IBGW 경로를 빠르고 신뢰성 있게 만들며, 전체 네트워크 연결을 줄입니다.

우리는 [TUNNEL-DELIVERY]_의 플래그에서 하나의 사용되지 않은 전달 유형(0x03)과 두 개의 남은 비트(0과 1)를 사용하여 이 기능을 구현할 수 있습니다.
...
- OBEP를 제어하고 NetDB에서 LeaseSets를 긁어모으는 적은 [TunnelId]_ / [RouterIdentity]_ 쌍을 검색함으로써 특정 목적지로 메시지가 보내지는지 이미 결정할 수 있습니다. 최악의 경우, TMDI에 여러 개의 리스가 존재하면 적의 데이터베이스에서 일치 항목을 찾는 속도가 더 빨라질 수 있습니다.
...
첫 번째 프래그먼트 전달 지침 [TUNNEL-DELIVERY]_은 다음과 같이 수정됩니다:
...
새로운 명세를 이해해야 하는 유일한 피어는 OBGWs와 OBEPs입니다. 대상 I2P 버전 [VERSIONS]_에 대한 정보를 기반으로 하는 아웃바운드 터널을 구축할 때, OBGWs는 호환 가능한 OBEPs를 선택해야 합니다:
