...
네트워크 배포 및 테스트 진행 중.
사소한 수정이 있을 수 있습니다.
공식 사양은 [SPEC](/docs/specs/implementation/)을 참조하십시오.


## 개요

이 문서는 [ECIES-X25519](/docs/specs/ecies/)에서 도입된 암호화 프리미티브를 사용하여 터널 빌드 메시지 암호화를 변경할 것을 제안합니다.
이는 라우터를 ElGamal에서 ECIES-X25519 키로 변환하는 전체 제안 [Prop156](/proposals/156-ecies-routers/)의 일부입니다.
...
ECIES 라우터에 필요한 전체 변경 사항의 개요는 제안 156 [Prop156](/proposals/156-ecies-routers/)를 참조하십시오.

이 제안서는 호환성을 위해 터널 빌드 레코드의 크기를 동일하게 유지합니다.
작은 빌드 레코드 및 메시지는 나중에 구현될 것입니다 - [Prop157](/proposals/157-new-tbm/)를 참조하십시오.
...
- [암호화](/docs/specs/cryptography/)에서처럼 AES-256-CBC
- STREAM ChaCha20/Poly1305 함수:
  [NTCP2](/docs/specs/ntcp2/), [ECIES-X25519](/docs/specs/ecies/) 및 [RFC-7539](https://tools.ietf.org/html/rfc7539)에서처럼 ENCRYPT(k, n, plaintext, ad) 및 DECRYPT(k, n, ciphertext, ad)
- [NTCP2](/docs/specs/ntcp2/) 및 [ECIES-X25519](/docs/specs/ecies/)에서처럼 X25519 DH 함수
- [NTCP2](/docs/specs/ntcp2/) 및 [ECIES-X25519](/docs/specs/ecies/)에서처럼 HKDF(salt, ikm, info, n)

다른 Noise 함수는 다른 곳에 정의됨:

- [NTCP2](/docs/specs/ntcp2/) 및 [ECIES-X25519](/docs/specs/ecies/)에서처럼 MixHash(d)
- [NTCP2](/docs/specs/ntcp2/) 및 [ECIES-X25519](/docs/specs/ecies/)에서처럼 MixKey(d)
...
- 터널 AES "레이어" 암호화 변경 없음, [Prop153](/proposals/153-chacha20-layer-encryption/) 참조
...
- [Prop143](/proposals/143-build-message-options/)에서 정의된 터널 빌드 옵션 사용, 작은 메시지에만 필요
- 양방향 터널 - [Prop119](/proposals/119-bidirectional-tunnels/) 참조
- 작은 터널 빌드 메시지 - [Prop157](/proposals/157-new-tbm/) 참조
...
이는 또한 [NTCP2](/docs/specs/ntcp2/)에서 사용된 XK 패턴의 첫 번째 (세션 요청) 메시지와 동일합니다.
...
참고로, 이것은 [I2NP](/docs/specs/i2np/)에서 가져온 ElGamal 라우터에 대한 터널 BuildRequestRecord의 현재 사양입니다.
...
플래그 필드는 [Tunnel-Creation](/docs/specs/tunnel-creation/)에서 정의된 것과 동일하며 다음을 포함합니다.::
...
터널 빌드 옵션은 [Common](/docs/specs/common-structures/)에서 정의된 매핑 구조입니다.
...
터널 빌드 응답 옵션은 [Common](/docs/specs/common-structures/)에서 정의된 매핑 구조입니다.
이는 향후 사용을 위한 것입니다. 현재 정의된 옵션은 없습니다.
매핑 구조가 비어 있는 경우, 이는 두 바이트 0x00 0x00입니다.
...
[Tunnel-Creation](/docs/specs/tunnel-creation/)에서 정의된 대로:
...
라우터 정적 ECIES 키의 세부사항은 [Prop156](/proposals/156-ecies-routers/)를 참조하십시오.
...
ECIES 터널 생성자는 각 ElGamal 홉의 공개 키를 사용하여 [Tunnel-Creation](/docs/specs/tunnel-creation/)에서 정의된 방식으로 암호화해야 합니다. ECIES 터널 생성자는 ECIES 홉에 대한 암호화를 위해 위의 방식을 사용할 것입니다.
...
[Prop156](/proposals/156-ecies-routers/)를 참조하십시오.
