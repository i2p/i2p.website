...
이 문서는 [ECIES-X25519]_에서 도입된 암호화 프리미티브를 사용하여 터널 빌드 메시지 암호화를 변경할 것을 제안합니다.
이는 라우터를 ElGamal에서 ECIES-X25519 키로 변환하는 전체 제안 [Prop156]_의 일부입니다.
...
ECIES 라우터에 필요한 전체 변경 사항의 개요는 제안 156 [Prop156]_를 참조하십시오.

이 제안서는 호환성을 위해 터널 빌드 레코드의 크기를 동일하게 유지합니다.
작은 빌드 레코드 및 메시지는 나중에 구현될 것입니다 - [Prop157]_를 참조하십시오.
...
- [암호화]_에서처럼 AES-256-CBC
- STREAM ChaCha20/Poly1305 함수:
  [NTCP2]_, [ECIES-X25519]_ 및 [RFC-7539]_에서처럼 ENCRYPT(k, n, plaintext, ad) 및 DECRYPT(k, n, ciphertext, ad)
- [NTCP2]_ 및 [ECIES-X25519]_에서처럼 X25519 DH 함수
- [NTCP2]_ 및 [ECIES-X25519]_에서처럼 HKDF(salt, ikm, info, n)

다른 Noise 함수는 다른 곳에 정의됨:

- [NTCP2]_ 및 [ECIES-X25519]_에서처럼 MixHash(d)
- [NTCP2]_ 및 [ECIES-X25519]_에서처럼 MixKey(d)
...
- 터널 AES "레이어" 암호화 변경 없음, [Prop153]_ 참조
...
- [Prop143]_에서 정의된 터널 빌드 옵션 사용, 작은 메시지에만 필요
- 양방향 터널 - [Prop119]_ 참조
- 작은 터널 빌드 메시지 - [Prop157]_ 참조
...
[NOISE]_ (Revision 34, 2018-07-11)을 기반으로 합니다.
...
- DH 함수: X25519
  [RFC-7748]_에서 지정된 32바이트 키 길이의 X25519 DH.

- 암호 함수: ChaChaPoly
  [RFC-7539]_ 2.8절에서 지정된 AEAD_CHACHA20_POLY1305.
...
이는 또한 [NTCP2]_에서 사용된 XK 패턴의 첫 번째 (세션 요청) 메시지와 동일합니다.
...
참고로, 이것은 [I2NP]_에서 가져온 ElGamal 라우터에 대한 터널 BuildRequestRecord의 현재 사양입니다.
...
플래그 필드는 [Tunnel-Creation]_에서 정의된 것과 동일하며 다음을 포함합니다.::
...
터널 빌드 옵션은 [Common]_에서 정의된 매핑 구조입니다.
...
터널 빌드 응답 옵션은 [Common]_에서 정의된 매핑 구조입니다.
이는 향후 사용을 위한 것입니다. 현재 정의된 옵션은 없습니다.
매핑 구조가 비어 있는 경우, 이는 두 바이트 0x00 0x00입니다.
...
응답 경로에서는, 엔드포인트 (송신자)는 [다중 암호화]_를 사용하여 각 홉의 응답 키를 사용해 해독해야 합니다.
...
라우터 정적 ECIES 키의 세부사항은 [Prop156]_를 참조하십시오.
...
ECIES 터널 생성자는 각 ElGamal 홉의 공개 키를 사용하여 [Tunnel-Creation]_에서 정의된 방식으로 암호화해야 합니다. ECIES 터널 생성자는 ECIES 홉에 대한 암호화를 위해 위의 방식을 사용할 것입니다.
