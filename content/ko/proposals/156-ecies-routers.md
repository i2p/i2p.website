...
LS2 [Prop123](/proposals/123-new-netdb-entries/)와 ECIES-X25519-AEAD-Ratchet [Prop144](/proposals/144-ecies-x25519-aead-ratchet/)의 제안은
목적지를 위해 ElGamal을 ECIES로 교체한다고 정의함
(현재 [ECIES](/docs/specs/ecies/)에 지정됨).
...
추가 목표는 [Prop152](/proposals/152-ecies-tunnels/) 참조.
...
추가 비목표는 [Prop152](/proposals/152-ecies-tunnels/) 참조.

- 이중 키 라우터 요구 없음
- 계층 암호화 변경 없음, 이에 대해서는 [Prop153](/proposals/153-chacha20-layer-encryption/) 참조
...
라우터에는 이러한 것이 필요하지 않음. 라우터의 암호화 키는 라우터 아이덴티티에 있음. 공통 구조 명세 [Common](/docs/specs/common-structures/) 참조.
...
이는 공통 구조 명세 [Common](/docs/specs/common-structures/)에서 정의한 표준 구성임.

이는 암호화 유형 1-3에 대해 제안 145 [Prop145](/proposals/145-ecies/)에서 제안된 ECIES-P256에 대한 방법과 동일함.
...
이 변경 사항은 제안 152 [Prop152](/proposals/152-ecies-tunnels/)에 정의됨.

2단계에서는 새로운 버전의 빌드 요청 메시지, 빌드 응답 메시지, 빌드 요청 레코드 및 빌드 응답 레코드를 추가할 예정임.
크기를 줄여 효율성을 높일 예정임.
이 변경 사항은 터널의 모든 홉에서 지원해야 하며, 모든 홉은 ECIES여야 함.
이 변경 사항은 제안 157 [Prop157](/proposals/157-new-tbm/)에 정의됨.
...
그래서, 우리는 ECIES-X25519-AEAD-Ratchet [Prop144](/proposals/144-ecies-x25519-aead-ratchet/),
지금은 [ECIES](/docs/specs/ecies/)에 지정되어 있는 ECIES Ratchet SKM을 설계함.
...
- 목적지를 위한 [ECIES](/docs/specs/ecies/)에 설명 된 "이중 키" 세션 키 관리자를 실행할 필요 없음. 라우터는 단일 공개 키만 가짐.


#### 설계 결론

ECIES 라우터 SKM은 목적지에 대해 [ECIES](/docs/specs/ecies/)에 지정된 전체 Ratchet SKM을 필요로 하지 않음.
IK 패턴을 사용하는 비익명 메시지에 대한 요구 사항이 없음.
위협 모델은 Elligator2로 인코딩된 일회용 키를 요구하지 않음.

따라서 라우터 SKM은 [Prop152](/proposals/152-ecies-tunnels/)에 터널 빌딩을 위해 지정한 것과 동일한 노이즈 "N" 패턴을 사용할 것임.
목적지에 대해 [ECIES](/docs/specs/ecies/)에서 지정한 것과 동일한 페이로드 형식을 사용할 것임.
IK에서 지정한 제로 고정 키(바인딩 또는 세션 없음) 모드는 사용하지 않을 것임.

탐색에 대한 응답은 탐색에서 요청한 경우 라쳇 태그로 암호화됨.
이는 [Prop154](/proposals/154-ecies-lookups/)에 문서화되어 있으며, 현재는 [I2NP](/docs/specs/i2np/)에 지정되어 있음.

이 설계에서는 라우터가 단일 ECIES 세션 키 관리자를 가지도록 함.
목적지를 위한 [ECIES](/docs/specs/ecies/)에 설명된 "이중 키" 세션 키 관리자를 실행할 필요가 없음.
라우터는 단일 공개 키만 가짐.
...
X25519: [ECIES](/docs/specs/ecies/) 참조.

라우터 아이덴티티 및 키 인증서: [Common](/docs/specs/common-structures/) 참조.

터널 빌딩: [Prop152](/proposals/152-ecies-tunnels/) 참조.

새로운 터널 빌드 메시지: [Prop157](/proposals/157-new-tbm/) 참조.


### 요청 암호화

요청 암호화는 [Tunnel-Creation-ECIES](/docs/specs/implementation/#tunnel-creation-ecies) 및 [Prop152](/proposals/152-ecies-tunnels/)에 지정된 것과 동일하며,
노이즈 "N" 패턴을 사용함.

탐색에 대한 응답은 탐색에서 요청한 경우 라쳇 태그로 암호화됨.
데이터베이스 탐색 요청 메시지는 [I2NP](/docs/specs/i2np/) 및 [Prop154](/proposals/154-ecies-lookups/)에 지정된 대로 32바이트 응답 키와 8바이트 응답 태그를 포함함.
응답을 암호화하는 데 키와 태그가 사용됨.

태그 세트는 생성되지 않음.
ECIES-X25519-AEAD-Ratchet [Prop144](/proposals/144-ecies-x25519-aead-ratchet/) 및 [ECIES](/docs/specs/ecies/)에 지정된
제로 고정 키 기법은 사용되지 않음.
...
이는 [Tunnel-Creation-ECIES](/docs/specs/implementation/#tunnel-creation-ecies) 및 [Prop152](/proposals/152-ecies-tunnels/)에 터널 빌드 메시지에 지정된 것과 동일함.
...
페이로드는 [ECIES](/docs/specs/ecies/) 및 [Prop144](/proposals/144-ecies-x25519-aead-ratchet/)에 정의된 동일 블록 형식임.
...
이는
[I2NP](/docs/specs/i2np/) 및 [Prop154](/proposals/154-ecies-lookups/)에 지정된 대로
...
제안 145 [Prop145](/proposals/145-ecies/)는 제안 152 [Prop152](/proposals/152-ecies-tunnels/)와 대부분 호환되게
...
이번에는 미완성 제안 145 [Prop145](/proposals/145-ecies/)로 인해.
...
제안 152 [Prop152](/proposals/152-ecies-tunnels/)에 정의된 터널 빌딩을 구현함.
...
제안 144 [Prop144](/proposals/144-ecies-x25519-aead-ratchet/)에 정의된 대로 진행합니다.
...
제안 157 [Prop157](/proposals/157-new-tbm/)에 정의된 새로운 터널 빌드 메시지를 구현하고 테스트합니다.
