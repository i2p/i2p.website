...
네트워크 배포 및 테스트 진행 중.
사소한 수정이 있을 수 있음.
공식 사양은 [specification](/en/docs/spec/red25519/)를 참조하세요.
...
[Proposal 123](/en/proposals/123-new-netdb-entries/)는 최소 권한 원칙을 구현한 암호화된 LeaseSet2 포맷을 정의합니다. 즉, 각 네트워크 참여자에게는 그들의 역할에 필요한 정보만 제공됩니다. 특히, 플로드필에 게시되는 암호화된 LeaseSet2는 그 목적지를 공개하지 않으며, 목적지에 대한 사전 지식이 있는 사람만 임대 정보를 볼 수 있습니다. 그러나 플로드필은 여전히 게시 시 암호화된 LeaseSet2를 인증할 수 있어야 하며, 클라이언트는 목적지 자체가 인증을 적용했는지 추가로 확인해야 합니다.
