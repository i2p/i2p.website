---
title: "SAM v3"
description: "بروتوكول جسر مستقر لتطبيقات I2P غير المبنية على Java"
slug: "samv3"
lastUpdated: "2025-10"
accurateFor: "2.10.0"
---

SAM v3 ("Simple Anonymous Messaging") هو **واجهة برمجية مستقرة ومستقلة عن الموجه** الحالية التي تتيح للتطبيقات الخارجية التواصل مع شبكة I2P دون الحاجة لتضمين الموجه نفسه. توفر وصولاً موحداً إلى **التدفقات**، و**مخططات البيانات**، و**الرسائل الخام**، وتبقى طبقة الربط المعيارية للبرمجيات غير المبنية بلغة Java.

## 1. نظرة عامة والغرض

يُمكّن SAMv3 المطورين من بناء برمجيات متوافقة مع I2P بأي لغة برمجة باستخدام بروتوكول TCP/UDP خفيف الوزن. يقوم بتجريد العمليات الداخلية لل router، ويكشف عن مجموعة صغيرة من الأوامر عبر TCP (7656) و UDP (7655). كل من **Java I2P** و **i2pd** يُطبّقان مجموعات فرعية من مواصفات SAMv3، على الرغم من أن i2pd لا يزال يفتقر إلى معظم إضافات 3.2 و 3.3 حتى عام 2025.
...
- **Java I2P** يستخدم `PRIMARY/SUBSESSION`.
- **i2pd** و **I2P+** يواصلان استخدام مصطلحات `MASTER/SUBSESSION` القديمة للتوافق مع الإصدارات السابقة.
...
```
- `SIGNATURE_TYPE=7` → **Ed25519 (EdDSA SHA512)**. موصى به بشدة منذ I2P 0.9.15.
...
```
- `i2cp.leaseSetEncType=4,0` → `4` هو X25519 (ECIES X25519 AEAD Ratchet) و `0` هو ElGamal احتياطي للتوافق.
- كميات الأنفاق الصريحة للاتساق: الافتراضي في Java I2P **2**، الافتراضي في i2pd **5**.
...
STREAM CLOSE ID=1
```
أنواع الرسائل الأساسية تشمل: `STREAM CONNECT`، `STREAM ACCEPT`، `STREAM FORWARD`، `DATAGRAM SEND`، `RAW SEND`، `NAMING LOOKUP`، `DEST LOOKUP`، `PING`، `QUIT`.
...
QUIT
```
## 4. الاختلافات في التنفيذ (Java I2P مقابل i2pd)
...
**التوصية:** حدد دائمًا كميات الأنفاق بشكل صريح لضمان الاتساق عبر أجهزة التوجيه (routers).

## 5. المكتبات المدعومة (لقطة 2025)
...
## 6. الميزات القادمة والجديدة (2024-2025)
...
## 7. ملاحظات الأمان والتكوين

- اربط SAM بـ `127.0.0.1` فقط.
- للخدمات الدائمة، استخدم جلسات **PRIMARY** مع مفاتيح ثابتة.
- استخدم `HELLO VERSION` لاختبار دعم الميزات.
- استخدم `PING` أو `NAMING LOOKUP` للتحقق من نشاط الـ router.
- تجنب اتصالات SAM البعيدة غير المصادق عليها (لا يوجد TLS في i2pd).

## 8. المراجع والمواصفات

- [مواصفات SAM v3](/docs/api/samv3/)
- [SAM v2 (قديم)](/docs/legacy/samv2/)
- [مواصفات البث المباشر](/docs/specs/streaming/)
- [Datagrams](/docs/api/datagrams/)
- [مركز التوثيق](/docs/)
- [توثيق i2pd](https://i2pd.website/docs)

## 9. الملخص

يظل SAM v3 هو **بروتوكول الجسر الموصى به** لجميع تطبيقات I2P غير المبنية على Java. فهو يوفر الاستقرار، وروابط متعددة اللغات، وأداءً متسقًا عبر أنواع router المختلفة.
