...
اطلع على [SPEC]_ للحصول على المواصفات الرسمية.

## النظرة العامة

يقترح هذا المستند تغييرات في تشفير رسائل بناء الأنفاق باستخدام البدائل التشفيرية المقدمة بواسطة [ECIES-X25519]_.
إنه جزء من الاقتراح العام [Prop156]_ لتحويل أجهزة التوجيه من مفاتيح ElGamal إلى مفاتيح ECIES-X25519.
...
للحصول على نظرة عامة على جميع التغييرات المطلوبة لأجهزة التوجيه ECIES، انظر الاقتراح 156 [Prop156]_.

يحافظ هذا الاقتراح على نفس حجم سجلات بناء الأنفاق،
حسب ما هو مطلوب للتوافق. سيتم تنفيذ سجلات ورسائل بناء أصغر لاحقًا - انظر [Prop157]_.

### البدائل التشفيرية

لا يتم تقديم أي بدائل تشفير جديدة. البدائل المطلوبة لتنفيذ هذا الاقتراح:

- AES-256-CBC كما في [Cryptography]_
- دوال STREAM ChaCha20/Poly1305:
  ENCRYPT(k, n, plaintext, ad) وDECRYPT(k, n, ciphertext, ad) - كما في [NTCP2]_ و[ECIES-X25519]_ و[RFC-7539]_
- دوال X25519 DH - كما في [NTCP2]_ و[ECIES-X25519]_
- HKDF(salt, ikm, info, n) - كما في [NTCP2]_ و[ECIES-X25519]_

دوال Noise الأخرى المعرفة في مكان آخر:

- MixHash(d) - كما في [NTCP2]_ و[ECIES-X25519]_
- MixKey(d) - كما في [NTCP2]_ و[ECIES-X25519]_
...
- لا تغيير في تشفير الطبقة AES للنفق، لذلك راجع [Prop153]_
...
- الأنفاق ثنائية الاتجاه - لذلك راجع [Prop119]_
- تقليل عدد الرسائل البناء النفق - لذلك راجع [Prop157]_
...
[NOISE]_ (المراجعة 34، 2018-07-11).
...
- وظيفة DH: X25519
  X25519 DH بطول مفتاح 32 بايت كما هو محدد في [RFC-7748]_.

- وظيفة تشفير: ChaChaPoly
  AEAD_CHACHA20_POLY1305 كما هو محدد في [RFC-7539]_ القسم 2.8.
  مفتاح غير متكرر 12 بايت، مع تعيين الأربعة بايت الأولى إلى الصفر.
  مطابقة لمتطلبات [NTCP2]_.
...
تستخدم التصافحات أنماط التصافح [Noise]_
...
وهذا مطابق أيضًا للرسالة الأولى (طلب الجلسة) في النمط XK المستخدم في [NTCP2]_.
...
يشمل التشفير غير المتماثل لسجلات الطلبات حاليًا ElGamal كما هو معرف في [Cryptography]_
...
للمراجعة، هذا هو المواصفات الحالية لـ tunnel BuildRequestRecord لأجهزة توجيه ElGamal، مأخوذ من [I2NP]_.
يتم إضافة بايت غير صفري وهش البيانات (SHA-256) إلى البيانات غير المشفرة قبل التشفير،
كما هو معرف في [Cryptography]_.
...
حقل الأعلام هو نفس ما تم تعريفه في [Tunnel-Creation]_ ويحتوي على:
...
خيارات بناء النفق هي بنية Mapping كما هو معرف في [Common]_.
...
خيارات الرد لبناء النفق هي بنية Mapping كما هو معرف في [Common]_.
هذا للاستخدام المستقبلي. لا توجد خيارات معرفة حاليًا.
...
كما هو معرف في [Tunnel-Creation]_ لتجنب البصمة:
...
في مسار الرد، سيحتاج الطرف الرئيسي (المرسل) إلى إلغاء [Multiple-Encryption]_, باستخدام مفتاح الرد الخاص بكل نقطة.
...
اطلع على [Prop156]_ للحصول على تفاصيل مفاتيح التوجيه الثابتة لـ ECIES.
...
هذا هو [NOISE]_ القياسي للنمط "N" مع اسم بروتوكول قياسي.
...
المخطط الذي تم تعريفه في [Tunnel-Creation]_. سيستخدم منشئو الأنفاق ECIES المخطط أعلاه للتشفير
إلى نقاط ECIES.
...
كما هو معرف في [Tunnel-Creation]_.
