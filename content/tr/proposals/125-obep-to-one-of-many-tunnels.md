...
Bu önerinin delege etme kısmına alternatif olarak, mevcut hedef [RouterIdentity]_ karmasını belirtme yeteneğine benzer şekilde, bir [LeaseSet]_ karması gönderilebilir. Bu, daha küçük bir mesaj ve potansiyel olarak daha yeni bir LeaseSet ile sonuçlanacaktır. Ancak:
...
Başlatıcı (OBGW), hedef [Leases]_'lerden bazılarını (tümünü?) teslimat talimatlarına [TUNNEL-DELIVERY]_ yerleştirir, sadece bir tane seçmek yerine.
...
Kullanılmayan bir teslimat türümüz (0x03) ve [TUNNEL-DELIVERY]_ bayraklarında iki kalan bitimiz (0 ve 1) var, bu özellikleri uygulamak için bunları kullanabiliriz.
...
- OBEP'yi kontrol eden ve NetDB'den LeaseSet'leri kazıyan bir saldırgan, bir mesajın belirli bir Destination'a gönderilip gönderilmediğini, [TunnelId]_ / [RouterIdentity]_ çiftini arayarak zaten belirleyebilir. En kötü ihtimalle, TMDI'deki birden fazla Lease varlığı, saldırganın veritabanında bir eşleşme bulma hızını artırabilir.
...
İlk Parça Teslimat Talimatları [TUNNEL-DELIVERY]_ şu şekilde değiştirilecektir:
...
Yeni spesifikasyonu anlaması gereken tek eşler OBGW'ler ve OBEP'lerdir. Dolayısıyla, bu değişikliği hedef I2P sürümü [VERSIONS]_ üzerinden mevcut ağ ile uyumlu hale getirebiliriz:
